...
parse_heuristic_with_arguments_from_braced_expression(string str) {
  string heuristic = "";
  size_t pos = 0;
  ...
  if (pos != str.size()) {
    ...
    for (string arg : args) {
      ...
      if (argElems.size() == 1)
        ...
        cout << "option " << arg << " has more than one equals sign ..."
             << endl;
        exit(1);
      }
      position++;
...
enum planningAlgorithm { PROGRESSION, SAT, BDD, INTERACTIVE, TRANSLATION };
...
void printHeuristicHelp() {
  ...
  cout << "  dof" << endl;
  cout << "" << endl;
  cout << "Arguments are given to the heuristics in normal braces. E.g. use "
          "cost(invert) to pass the argument 'invert' to the heuristic 'cost'."
       << endl;
  cout << "We explain below which arguments (like 'invert') exist for each "
          "heuristic. Multiple arguments must be separated by semicolons. "
       << endl;
  cout << "By default the arguments can just be given in order. It is also "
          "possible to provide them using the key=value syntax. The name"
       << endl;
  cout << "of the argument is given in braces" << endl;
  cout << "" << endl;
  cout << "- zero is a heuristic that returns always zero." << endl;
  cout << "- cost is a heuristic that return the action costs of the actions "
          "taken leading to the current search node"
       << endl;
  cout << "  (not including actions present in the current (remaining) task "
          "network)"
       << endl;
  cout << "- modDepth and mixedModDepth are as explained in the planner "
          "helptext, as possible values for -g"
       << endl;
  cout << "- The modDepth, mixedModDepth, and cost heuristics all have only "
          "one optional argument \"invert\" (or invert=true)."
       << endl;
  cout << "  If provided the heuristic value will be multiplied with -1."
       << endl;
  cout << "" << endl;
  cout << "- The rc2 heuristic has the following arguments:" << endl;
  cout << "  1. The inner heuristic (key: h). If you don't provide one, this "
          "will default to ff. You may choose one of"
       << endl;
  cout << "    - ff (the FF heuristic)" << endl;
  cout << "    - add (the additive heuristic)" << endl;
  cout << "    - lmc (the LM-cut heuristic)" << endl;
  cout << "    - filter (0 if the HTN goal is reachable under delete "
          "relaxation, infinity otherwise)"
       << endl;
  cout << "  2. What to estimate (key: est). There are three possible values. "
          "The default is distance"
       << endl;
  cout << "    - distance (estimate number of actions and methods to apply)"
       << endl;
  cout << "    - cost (estimate cost of actions to apply)" << endl;
  cout << "    - mixed (estimate cost of actions plus number of methods to "
          "apply)"
       << endl;
  cout << "  3. Correct Task Count (key: taskcount). This option is on by "
          "default."
       << endl;
  cout << "     If on, the heuristic is improved by counting the minimally "
          "implied cost/count "
       << endl;
  cout << "     of actions that occur more than once in the current task "
          "network."
       << endl;
  cout << "     To turn it off, pass no as the third argument" << endl;
  cout << "" << endl;
  cout << "- The dof (delete- and order-free relaxation) heuristic requires "
          "that pandaPIengine is compiled with CPLEX support."
       << endl;
  cout << "  It has the following 8 arguments." << endl;
  cout << "  1. Type (key: type). Default is ilp. Can be set to lp. Determines "
          "whether the problem is solved as an ILP or LP"
       << endl;
  cout << "  2. Mode (key: mode). Default is satisficing. Can be set to "
          "optimal to make the heuristic optimal."
       << endl;
  cout << "  3. TDG (key: tdg). Default allowUC (allow unconnected). Can be "
          "set to uc (full encoding, no unconnected parts) or none."
       << endl;
  cout << "  4. PG (key: pg). Default is none. Can be set to full or relaxed "
          "to enable full or relaxed planning graph."
       << endl;
  cout << "  5. And/Or Landmarks (key: andOrLM). Default is None." << endl;
  cout << "     Can be set to full or onlyTNi to include all landmarks or only "
          "those derivable from the initial task network"
       << endl;
  cout << "  6. External landmarks (key: externalLM). Default is no. With none "
          "can be set to use external landmarks."
       << endl;
  cout << "  7. LM-cut (key: lmclmc). Default is full. Can be set to none to "
          "not include LM-cut landmarks"
       << endl;
  cout << "  8. Netchange Constraints (key: netchange). Default is full. Can "
          "be set to none to not include net-change constraints"
       << endl;
}
...
int main(int argc, char *argv[]) {
  ...
  if (inputFiles.size() > 1) {
    std::cerr << "You may specify at most one file as input: the SAS+ problem "
                 "description"
              << std::endl;
    return 1;
  } else {
  ...
  if (args_info.satmutexes_flag)
    htn->rintanenInvariants = true;
  ...
  if (args_info.writeInputToHDDL_given) {
    ...
    if (inputFilename == "-") {
      cout << "Cannot determine proper file names when reading from stdin"
           << endl;
      return 1;
    }
  ...
  if (algo == INTERACTIVE) {
    ...
    for (int i = 0; i < hLength; i++) {
      auto [hName, args] =
          parse_heuristic_with_arguments_from_braced_expression(
              args_info.heuristic_arg[i]);

      if (heuristics_so_far.count({hName, args})) {
        heuristics[i] = heuristics[heuristics_so_far[{hName, args}]];
        cout << "\tHeuristic duplicate: Nr. " << i << " is the same as "
             << heuristics_so_far[{hName, args}] << endl;
        continue;
      }
      ...
      if (hName == "zero") {
        ...
        string invertString =
            (args.count("invert")) ? args["invert"] : args["arg1"];
        bool invert = false;
        if (invertString == "true" || invertString == "invert")
          invert = true;
        ...
        string estimate_string =
            (args.count("est")) ? args["est"] : args["arg2"];
        ...
        string correct_task_count_string =
            (args.count("taskcount")) ? args["taskcount"] : args["arg3"];
        ...
        string andorLM_string =
            (args.count("andOrLM")) ? args["andOrLM"] : args["arg5"];
        ...
        string externalLM_string =
            (args.count("externalLM")) ? args["externalLM"] : args["arg6"];
        csAddExternalLms externalLM = cAddExternalLmsNo;
        if (externalLM_string == "none")
          externalLM = cAddExternalLmsYes;

        string lmclms_string =
            (args.count("lmclmc")) ? args["lmclmc"] : args["arg7"];
        csLmcLms lmclms = cLmcLmsFull;
        if (lmclms_string == "none")
          lmclms = cLmcLmsNone;

        string netchange_string =
            (args.count("netchange")) ? args["netchange"] : args["arg8"];
        csNetChange netchange = cNetChangeFull;
        if (netchange_string == "none")
          netchange = cNetChangeNone;

        heuristics[i] =
            new hhDOfree(htn, tnI, i, intType, boolType, mode, tdg, pg, andOrLM,
                         lmclms, netchange, externalLM);
#else
      ...
      cout << "Heuristic #" << i << " = " << heuristics[i]->getDescription()
           << endl;
    }

    int aStarWeight = args_info.astarweight_arg;
    aStar aStarType = gValNone;
    if (string(args_info.gValue_arg) == "path")
      aStarType = gValPathCosts;
    if (string(args_info.gValue_arg) == "action")
      aStarType = gValActionCosts;
    if (string(args_info.gValue_arg) == "mixed")
      aStarType = gValActionPathCosts;
    if (string(args_info.gValue_arg) == "none")
      aStarType = gValNone;
    ...
    search.search(htn, tnI, timeL, suboptimalSearch, printPlan, heuristics,
                  hLength, visi, fringe);
  } else if (algo == SAT) {
#ifndef CMAKE_NO_SAT
    ...
    bool effectLessActionsInSeparateLeaf =
        args_info.methodPreconditionsInSeparateLeafs_given;
    bool optimisingMode = args_info.optimisation_given;

    sat_pruning pruningMode = SAT_FF;
    if (string(args_info.pruning_arg) == "none")
      pruningMode = SAT_NONE;
    if (string(args_info.pruning_arg) == "ff")
      pruningMode = SAT_FF;
    if (string(args_info.pruning_arg) == "h2")
      pruningMode = SAT_H2;
    ...
    solve_with_sat_planner(htn, block_compression, sat_mutexes, pruningMode,
                           effectLessActionsInSeparateLeaf, optimisingMode);
#else
    ...
    if (string(args_info.transtype_arg) == "push")
      type = Push;
    if (string(args_info.transtype_arg) == "parallelseq")
      type = ParallelSeq;
    if (string(args_info.transtype_arg) == "to")
      type = TO;
    if (string(args_info.transtype_arg) == "postrips")
      type = BaseStrips;
    if (string(args_info.transtype_arg) == "pocond")
      type = BaseCondEffects;

    runTranslationPlanner(
        htn, type, args_info.forceTransType_flag, args_info.pgb_arg,
        args_info.pgbsteps_arg, string(args_info.downward_arg),
        string(args_info.downwardConf_arg), string(args_info.sasfile_arg),
...
/*
    ...
    if(lmc->numLMs != ao->getNumLMs()) {
        cout << "AAAAAAAAAAAAAAAAAAAAHHH " << ao->getNumLMs() << " - " <<
   lmc->numLMs << endl; for(int i = 0; i < ao->getNumLMs(); i ++) {
            ao->getLMs()[i]->printLM();
        }
        cout << "----------------------" << endl;
    ...
    cout << "TIME:" << endl;
    cout << "Init       : " << initO << " " << initN << " delta " << (initN -
   initO) << endl; cout << "Generation : " << genO << " " << genN << " delta "
   << (genN - genO) << endl; cout << "Total      : " << (initO + genO) << " " <<
   (initN + genN) << " delta " << ((initN + genN) - (initO + genO)) << endl;
