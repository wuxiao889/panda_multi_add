...
searchNode::~searchNode() {
  ...
  // todo: need to destroy heuristic payload. To do so, I need to know the
  // number of heuristics used
}

void searchNode::printDFS(planStep *s, map<planStep *, int> &psp,
...
void searchNode::printNode(std::ostream &out) {
  out << "Node: " << this << endl;
  for (int a = 0; a < this->numAbstract; a++)
    cout << "\tUC A: " << this->unconstraintAbstract[a] << endl;
  for (int a = 0; a < this->numPrimitive; a++)
    cout << "\tUV P: " << this->unconstraintPrimitive[a] << endl;

  map<planStep *, int> psp;
  set<pair<planStep *, planStep *>> orderpairs;
  for (int a = 0; a < this->numAbstract; a++)
    this->printDFS(this->unconstraintAbstract[a], psp, orderpairs);
  for (int a = 0; a < this->numPrimitive; a++)
    this->printDFS(this->unconstraintPrimitive[a], psp, orderpairs);

  // names
  map<int, planStep *> bpsp;
  for (auto [a, b] : psp)
    bpsp[b] = a;
  for (int i = 0; i < bpsp.size(); i++)
    out << "\t" << setw(2) << i << " " << bpsp[i] << " " << bpsp[i]->task
        << endl;

  // ordering
  for (auto [a, b] : orderpairs)
    out << "\t" << setw(2) << psp[a] << " < " << setw(2) << psp[b] << endl;
}
...
pair<string, int> extractSolutionFromSearchNode(Model *htn, searchNode *tnSol) {
  ...
  while (!done) {
    ...
    if (sost->method >= 0) {
      ...
      application.second =
          htn->taskNames[sost->task] + " -> " + htn->methodNames[sost->method];
      decompositionStructure.push_back(application);
      if (sost->task == htn->initialTask)
    ...
    if (sost->mySolutionStepInstanceNumber != 0)
      children[sost->parentSolutionStepInstanceNumber].push_back(make_pair(
          sost->myPositionInParent, sost->mySolutionStepInstanceNumber));
