...
hhDOfree::hhDOfree(Model *htn, searchNode *n, int index,
                   IloNumVar::Type IntType, IloNumVar::Type BoolType,
                   csSetting IlpSetting, csTdg tdgConstrs, csPg pgConstrs,
                   csAndOrLms aoLMConstrs, csLmcLms lmcLMConstrs,
                   csNetChange ncConstrs, csAddExternalLms addLMConstrs)
    : Heuristic(htn, index), cIntType(IntType), cBoolType(BoolType),
      cSetting(IlpSetting), cTdg(tdgConstrs), cPg(pgConstrs),
      cAndOrLms(aoLMConstrs), cLmcLms(lmcLMConstrs), cNetChange(ncConstrs),
  ...
  if ((cIntType == IloNumVar::Int) && (cBoolType == IloNumVar::Bool)) {
    cout << "- ILP calculation: Integer and Boolean variables in the ILP are "
            "not relaxed"
         << endl;
  } else if ((cIntType == IloNumVar::Float) &&
             (cBoolType == IloNumVar::Float)) {
    cout << "- LP calculation: Integer and Boolean variables in the ILP are "
            "relaxed to Float variables"
         << endl;
  } else {
    if (cIntType == IloNumVar::Float) {
      cout << "- Integer variables in the ILP are relaxed to Float variables"
           << endl;
    ...
    if (cBoolType == IloNumVar::Float) {
      cout << "- Boolean variables in the ILP are relaxed to Float variables"
           << endl;
  ...
  if (this->cTdg == cTdgFull) {
    cout << "- initializing data structures to prevent disconnected "
            "components... ";
    ...
    for (int i = 0; i < htn->numCyclicSccs; i++) {
      ...
      for (int iTo = 0; iTo < htn->sccSize[scc]; iTo++)
        reachability[iTo] = new set<int>[htn->sccSize[scc]];
      ...
      for (int iF = 0; iF < htn->sccSize[scc]; iF++) {
        ...
        for (int iT = 0; iT < htn->sccSize[scc]; iT++) {
          ...
          sccNumInnerFromToMethods[scc][iF][iT] = reachability[iF][iT].size();
          l = 0;
          for (std::set<int>::iterator it = reachability[iF][iT].begin();
               it != reachability[iF][iT].end(); it++) {
            sccInnerFromToMethods[scc][iF][iT][l] = *it;
            // cout << "reaching " << htn->taskNames[to] << " from " <<
            // htn->taskNames[from] << " by " <<
            // htn->methodNames[sccInnerFromToMethods[from][to][l]] << endl;
            l++;
          }
        }
      }
  ...
  if (this->cNetChange == cNetChangeFull) {
    cout << "  - initializing data structures for net change constraints"
         << endl;
    ...
    for (int a = 0; a < htn->numActions; a++) {
      for (int iE = 0; iE < htn->numAdds[a]; iE++) {
        ...
        if (prec >= 0) {
          tAP[eff].insert(a);
          tAC[prec].insert(a);
          // cout << htn->taskNames[a] << " " << htn->factStrs[prec] << " -> "
          // << htn->factStrs[eff] << endl;
          continue;
        }
  ...
  if (this->cLmcLms == cLmcLmsFull) {
    hRC = new hhRC2<hsLmCut>(htn, 0, estDISTANCE,
                             false); // todo: which index to use?
  }
  if ((this->cAndOrLms == cAndOrLmsFull) ||
      (this->cAndOrLms == cAndOrLmsOnlyTnI)) {
...
void hhDOfree::printHeuristicInformation() {
  string s = "dof";
  if ((this->cBoolType == IloNumVar::Bool) &&
      (this->cIntType == IloNumVar::Int)) {
    s = s + "IP,";
  } else if ((this->cBoolType == IloNumVar::Float) &&
             (this->cIntType == IloNumVar::Float)) {
    s = s + "LP,";
  } else {
    exit(-1);
  }
...
int hhDOfree::recreateModel(searchNode *n) {
  ...
  for (int i = 0; i < htn->gSize;
       i++) { // make sure goal is reachable (not tested below)
  ...
  for (int i = 0; i < n->numContainedTasks;
       i++) { // is something bottom-up unreachable?
  ...
  if (!tnInonempty) { // fact-based goal has been checked before -> this is a
                      // goal node
    return 0;
  }
  ...
  if ((this->cPg == cPgTimeRelaxed) || (this->cPg == cPgFull)) {
    ...
    for (int a = pg->reachableTasksSet.getFirst();
         (a >= 0) && (a < htn->numActions);
         a = pg->reachableTasksSet.getNext()) {
      v.add(IloNumVar(lenv, 0, htn->numActions, cIntType));
      iTA[a] = iv;
#ifdef NAMEMODEL
  ...
  for (int i = pg->reachableTasksSet.getFirst();
    ...
    if (this->cSetting ==
        cOptimal) { // if satisficing: use unit-costs, aka modification
                    // distance, else use action costs
  ...
  if (this->cSetting ==
      cSatisficing) { // add other modifications, i.e., methods
    costs = 1;
    for (int i = pg->reachableMethodsSet.getFirst(); i >= 0;
         i = pg->reachableMethodsSet.getNext()) {
    ...
    for (int a = pg->reachableTasksSet.getFirst();
      ...
      for (int i = 0; i < htn->numPrecs[a]; i++) {
        ...
        if (pg->usefulFactSet.get(prec))
          model.add(100 * v[iUF[prec]] >=
                    v[iUA[a]]); // TODO: how large must the const be?
      }
      // C3
    ...
    if (this->cPg == cPgFull) {
      ...
      for (int a = pg->reachableTasksSet.getFirst();
        ...
        for (int iadd = 0; iadd < htn->numAdds[a]; iadd++) {
          ...
          model.add(v[iTA[a]] + 1 <=
                    v[iTP[add]] +
                        (htn->numActions + 1) * (1 - v[iE[EStartI[a] + iadd]]));
        }
      }
    }
  }
  ...
  // HTN stuff
  std::vector<IloExpr> tup(
      htn->numTasks,
      IloExpr(
          lenv)); // every task has been produced by some method or been in tnI
  ...
  if (this->cTdg == cTdgFull) { // prevent disconnected cycles
    for (int iSCC = 0; iSCC < htn->numCyclicSccs; iSCC++) {
      ...
      for (int iTask = 0; iTask < htn->sccSize[scc]; iTask++) {
        ...
        IloExpr c9(lenv);
        int iOfT =
            iu.indexOf(n->containedTasks, 0, n->numContainedTasks - 1, task);
        if (iOfT >= 0) {
      ...
      for (int iTime = 1; iTime < htn->sccSize[scc]; iTime++) {
        ...
        // create variables for ILP (needs to be done first, there may be
        // forward references)
        for (int iTask = 0; iTask < htn->sccSize[scc]; iTask++) {
          int task = htn->sccToTasks[scc][iTask];
          if (!pg->taskReachable(task))
          ...
          v.add(IloNumVar(lenv, 0, INT_MAX, cIntType));
#ifdef NAMEMODEL
          v[iv].setName(("R_" + to_string(scc) + "_" + to_string(iTime) + "_" +
                         to_string(iTask))
                            .c_str());
#endif
          ...
          for (int iTask2 = 0; iTask2 < htn->sccSize[scc]; iTask2++) {
            ...
            v.add(IloNumVar(lenv, 0, INT_MAX, cIntType));
#ifdef NAMEMODEL
            v[iv].setName(("I_" + to_string(scc) + "_" + to_string(iTime) +
                           "_" + to_string(iTask) + "_" + to_string(iTask2))
                              .c_str());
#endif
        ...
        for (int iTask = 0; iTask < htn->sccSize[scc];
             iTask++) { // t in the paper
          // C10
          int task = htn->sccToTasks[scc][iTask];
          if (!pg->taskReachable(task))
            continue;
          ...
          for (int iTask2 = 0; iTask2 < htn->sccSize[scc];
               iTask2++) { // t' in the paper
            int task2 = htn->sccToTasks[scc][iTask2];
            if (!pg->taskReachable(task2))
              continue;
            ...
            IloExpr c12(lenv);
            c12 = c12 + 0;
            for (int k = 0; k < sccNumInnerFromToMethods[scc][iTask2][iTask];
                 k++) {
      ...
      for (int iTask = 0; iTask < htn->sccSize[scc]; iTask++) {
        ...
        if (!pg->taskReachable(task))
          continue;
        model.add(v[iUA[task]] <=
                  v[RlayerCurrent[iTask]] *
                      100); // todo: what is a sufficiently high number?
      }
    }
  ...
  if ((this->cAndOrLms == cAndOrLmsOnlyTnI) ||
    ...
    for (int i = 0; i < causalLMs->getNumLMs(); i++) {
      ...
      if (lm->type == fact) {
        ...
        if ((lm->connection == atom) && (!n->state[f]) &&
            (pg->usefulFactSet.get(f))) {
          model.add(v[iUF[f]] >= 1);
        }
  ...
  if (this->cAndOrLms == cAndOrLmsFull) { // add all implications
    for (int t = pg->reachableTasksSet.getFirst(); t >= 0;
         t = pg->reachableTasksSet.getNext()) {
  ...
  if (this->cNetChange == cNetChangeFull) {
    for (int f = 0; f < htn->numStateBits; f++) {
      ...
      bool ovEqVInG =
          false; // variable where v belongs to is set to other value in G
      if (!vEqVInG) {
        int var = vOfL[f];
        for (int i = htn->firstIndex[var]; i < htn->lastIndex[var]; i++) {
      ...
      bool VnotInG =
          (!vEqVInG &&
           !ovEqVInG); // the entire variable v belongs to is not contained in G
      int l;
      if (n->state[f]) {
        if ((VnotInG) || (ovEqVInG)) {
          l = -1;
  ...
  // cplex.setParam(IloCplex::Param::TimeLimit, time / CHECKAFTER); // for some
  // time cplex.setParam(IloCplex::MIPEmphasis, CPX_MIPEMPHASIS_FEASIBILITY); //
  // focus on feasability cplex.setParam(IloCplex::MIPEmphasis,
  // IloCplex::MIPEmphasisType::MIPEmphasisFeasibility); // focus on feasability
  // cplex.setParam(IloCplex::IntSolLim, 1); // stop after first integer
  // solution
  ...
  if (cplex.solve()) {
    ...
    for (int i = pg->reachableTasksSet.getFirst(); (i >= 0) && (i <
    htn->numActions); i = pg->reachableTasksSet.getNext()) { double d =
    cplex.getValue(v[iUA[i]]); if (d > 0.000001) { cout <<  "T " <<
    htn->taskNames[i] << " == " << d << endl;
        }
    }
