...
enum csSetting { cSatisficing, cOptimal };
enum csTdg { cTdgFull, cTdgAllowUC, cTdgNone };
enum csPg { cPgFull, cPgTimeRelaxed, cPgNone };
enum csAndOrLms { cAndOrLmsNone, cAndOrLmsOnlyTnI, cAndOrLmsFull };
enum csAddExternalLms { cAddExternalLmsYes, cAddExternalLmsNo };
...
class hhDOfree : public Heuristic {
  ...
  int ***sccIncommingMethods =
      nullptr; // methods from another scc reaching a certain scc
  int ***sccInnerFromTo =
      nullptr; // maps a tasks to those that it may be decomposed into
  int ***sccInnerToFrom =
      nullptr; // maps a tasks to those that may be decomposed into it
...
public:
  hhDOfree(Model *htn, searchNode *n, int index, IloNumVar::Type IntType,
           IloNumVar::Type BoolType, csSetting IlpSetting, csTdg tdgConstrs,
           csPg pgConstrs, csAndOrLms aoLMConstrs, csLmcLms lmcLMConstrs,
  ...
  string getDescription() {
    return string("hhDOf(") +
           ...
           ";" + "tdg=" +
           (cTdg == cTdgFull ? "full"
                             : (cTdg == cTdgAllowUC ? "with-uc" : "none")) +
           ";" + "pg=" +
           (cPg == cPgFull
                ? "full"
                : (cPg == cPgTimeRelaxed ? "time-relaxed" : "none")) +
           ";" + "andOrLms=" +
           (cAndOrLms == cAndOrLmsFull
                ? "full"
                : (cAndOrLms == cAndOrLmsOnlyTnI ? "onlyTNi" : "none")) +
           ";" + "externalLms=" +
           (cAddExternalLms == cAddExternalLmsYes ? "yes" : "no") + ";" +
