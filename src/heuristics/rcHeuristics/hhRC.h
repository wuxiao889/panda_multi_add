...
private:
   ...
   */
  int t2tdr(int task) { return m->numStateBits + task; }

  int t2bur(int task) { return m->numStateBits + m->numActions + task; }
...
public:
  ...
  hhRC(Model *htn, Model *heuristicModel, int index, eEstimate estimate,
       bool correctTaskCount)
      : Heuristic(htn, index), estimate(estimate),
