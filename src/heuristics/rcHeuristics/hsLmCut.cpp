...
*/
  ...
  while (hMax > 0) {
    ...
    for (int cutted = cut->getFirst(); cutted >= 0; cutted = cut->getNext()) {
...
int hsLmCut::getHMax(bucketSet &s, noDelIntSet &g) {
  ...
  while (!heap->isEmpty()) {
    ...
    for (int iOp = 0; iOp < m->precToActionSize[prop]; iOp++) {
      ...
      if ((maxPrec[op] == UNREACHABLE) || (hVal[maxPrec[op]] < hVal[prop])) {
        maxPrec[op] = prop;
      }
      if (--unsatPrecs[op] == 0) {
...
int hsLmCut::updateHMax(noDelIntSet &g, bucketSet *cut) {
  ...
  for (int op = cut->getFirst(); op >= 0; op = cut->getNext()) {
    for (int iF = 0; iF < m->numAdds[op]; iF++) {
      int f = m->addLists[op][iF];
      if ((hVal[maxPrec[op]] + costs[op]) <
          hVal[f]) { // that f might be cheaper now
        hVal[f] = hVal[maxPrec[op]] + costs[op];
        heap->add(hVal[f], f);
      }
    }
  ...
  while (!heap->isEmpty()) {
    ...
    for (int iOp = 0; iOp < m->precToActionSize[prop]; iOp++) {
      int op = m->precToAction[prop][iOp];
      if ((unsatPrecs[op] == 0) &&
          (prop == maxPrec[op])) { // this may change the costs of the operator
