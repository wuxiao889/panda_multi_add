...
enum eEstimate { estDISTANCE, estMIXED, estCOSTS };
...
public:
  ...
  hhRC2(Model *htnModel, int index, eEstimate estimate, bool correctTaskCount)
      : Heuristic(htnModel, index), estimate(estimate),
        correctTaskCount(correctTaskCount) {

    Model *heuristicModel;
    factory = new RCModelFactory(htnModel);
    // two weird things:
    // 1. the model may contain artificial actions (for method preconditions),
    // which always have cost 0 If we estimate the distance, we may want to
    // count them.
    // 2. the original, non-artificial actions may have costs > 1 and for
    // distance we might not want to count them as something > 1

    // the first argument controls the cost of method actions
    // 0: all have cost 0
    // 1: all have cost 1
    // the second argument controls the cost of the primitive actions
    // 0: keep the costs of the input model (including the weird things
    // mentioned above) 1: set the costs of all actions to 1
    if (estimate == estCOSTS) {
      cout << "Generate 1" << endl << endl;
      heuristicModel =
          factory->getRCmodelSTRIPS(0, 0); // costs of methods need to be zero
    } else if (estimate == estMIXED) {
      cout << "Generate 2" << endl << endl;
      heuristicModel = factory->getRCmodelSTRIPS(
          1,
          0); // estimate distance -> method costs 1, actions keep their costs
    } else if (estimate == estDISTANCE) {
      cout << "Generate 3" << endl << endl;
      heuristicModel = factory->getRCmodelSTRIPS(
          1, 1); // estimate distance -> method costs 1, actions costs 1
    } else {
      cout << "BLUUUUUP";
      exit(0);
    }
    ...
    if (typeid(ClassicalHeuristic) == typeid(hsLmCut)) {
      ...
      if (useAdmissibleCostEstimate) {
        cout << "- a combination of cost estimate and admissible heuristic is "
                "used -> using admissible correction of heuristic value (this "
                "is bad when you do not want to find optimal plans)."
             << endl;
      }
    }
    if (storeCuts) {
      if (typeid(ClassicalHeuristic) != typeid(hsLmCut)) {
        useAdmissibleCostEstimate =
            (correctTaskCount && (estimate == estCOSTS));
        if (useAdmissibleCostEstimate) {
          cout << "- a combination of cost estimate and admissible heuristic "
                  "is used -> using admissible correction of heuristic value "
                  "(this is bad when you do not want to find optimal plans).";
        }
        storeCuts = false;
        cout << "- the option \"store cuts\" of the RC heuristic can only be "
                "used with the inner heuristic LM-Cut. It will be disabled."
             << endl;
      }
    }
  ...
  string getDescription() {
    return "hhRC2(" + sasH->getDescription() + ";" +
           (estimate == estDISTANCE
                ? "distance"
                : (estimate == estCOSTS ? "cost" : "mixed")) +
    ...
    if ((storeCuts) && (hval != UNREACHABLE)) {
      ...
      for (LMCutLandmark *storedcut : *cuts) {
        ...
        /*for (int i = 0; i < storedcut->size; i++) {
            if ((i > 0) && (storedcut->lm[i] >= htn->numActions) &&
        (storedcut->lm[i - 1] < htn->numActions)) cout << "| "; cout <<
        storedcut->lm[i] << " ";
        }
