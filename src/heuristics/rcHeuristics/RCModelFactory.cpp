...
RCModelFactory::RCModelFactory(Model *htn) { this->htn = htn; }
...
Model *RCModelFactory::getRCmodelSTRIPS(int costsMethodActions,
  ...
  for (int im = 0; im < htn->numMethods; im++) {
    int ia = htn->numActions + im;
    rc->taskNames[ia] =
        htn->methodNames[im] + "@" + htn->taskNames[htn->decomposedTask[im]];
  }
  ...
  for (int i = 0; i < htn->numMethods; i++) {
    for (int j = 0; j < htn->numSubTasks[i]; j++) {
      ...
      assert(contained); // is subtask contained in the respective action's
                         // preconditions?
    }
  }
 ...
 */
int RCModelFactory::t2tdr(int task) { return htn->numStateBits + task; }
...
pair<int, int> RCModelFactory::rcStateFeature2HtnIndex(string s) {
  ...
  s = s.substr(0,
               s.length() - 2); // ends with "()" due to grounded representation
  int type = -1;
  int index = -1;
    ...
    for (int i = 0; i < htn->numTasks; i++) {
      if (s.compare(su.toLowerString(su.cleanStr(htn->taskNames[i]))) == 0) {
        ...
        for (int j = i + 1; j < htn->numTasks; j++) {
          assert(s.compare(su.toLowerString(su.cleanStr(htn->taskNames[j]))) !=
                 0);
        }
#endif
        break;
    ...
    for (int i = 0; i < htn->numStateBits; i++) {
      if (s.compare(su.toLowerString(su.cleanStr(htn->factStrs[i]))) == 0) {
        ...
        for (int j = i + 1; j < htn->numStateBits; j++) {
          assert(s.compare(su.toLowerString(su.cleanStr(htn->factStrs[j]))) !=
                 0);
        }
#endif
        break;
