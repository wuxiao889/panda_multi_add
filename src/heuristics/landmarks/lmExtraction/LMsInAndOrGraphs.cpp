...
LMsInAndOrGraphs::LMsInAndOrGraphs(Model *htn) {
  ...
  if (!loopback)
    ...
    numNodes =
        htn->numStateBits + htn->numTasks + htn->numMethods + htn->numActions;
...
void LMsInAndOrGraphs::generateAndOrLMs(searchNode *tnI) {
  ...
  while (!heap->isEmpty()) {
    ...
    if (nodeType[n] == tAND) { // union of all parent sets
      tFullSet = false;
      for (vector<int>::iterator it = Ninv[n].begin(); it != Ninv[n].end();
           ++it) {
        int n2 = *it;
        if (fullSet[n2]) {
          tFullSet = true;
          ...
          for (set<int>::iterator it2 = LMs[n2].begin(); it2 != LMs[n2].end();
               ++it2) {
            int lm = *it2;
            temp->insert(lm);
          }
...
int LMsInAndOrGraphs::fNode(int i) { return i; }

int LMsInAndOrGraphs::aNode(int i) { return htn->numStateBits + i; }

int LMsInAndOrGraphs::mNode(int i) {
  return htn->numStateBits + htn->numTasks + i;
}

int LMsInAndOrGraphs::nodeToF(int i) { return i; }

int LMsInAndOrGraphs::nodeToA(int i) { return i - htn->numStateBits; }

int LMsInAndOrGraphs::nodeToM(int i) {
  return i - (htn->numStateBits + htn->numTasks);
}

bool LMsInAndOrGraphs::isFNode(int i) { return (i < htn->numStateBits); }
...
void LMsInAndOrGraphs::generateLocalLMs(Model *htn, searchNode *tnI) {
  ...
  for (set<int>::iterator it = initialTasks.begin(); it != initialTasks.end();
       ++it) {
    int task = *it;
    set<int> *newLMs = genLocalLMs(htn, task);
    for (set<int>::iterator it2 = newLMs->begin(); it2 != newLMs->end();
         ++it2) {
      collect->insert(*it2);
      lms->insert(*it2);
    }
  ...
  while (!collect->empty()) {
    ...
    collect = new set<int>();
    for (set<int>::iterator it = lastRound->begin(); it != lastRound->end();
         ++it) {
