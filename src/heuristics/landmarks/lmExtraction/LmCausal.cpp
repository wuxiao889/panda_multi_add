...
void LmCausal::copyGlobalLMsToResultSet(const searchNode *tn) {
  ...
  for (int i = 0; i < tn->numContainedTasks; i++) {
    // cout << "- " << tn->containedTasks[i] << " node" <<
    // tNode(tn->containedTasks[i]) << " "
...
bool LmCausal::setUnion(LmAoNode *update, int *combine, int size) {
  ...
  if (containsFullSet) {
    ...
    changed = !((!update->containsFullSet) && (update->numLMs == 1) &&
  ...
  if (!update->containsFullSet) {
    ...
    for (int lm : DEBUGres)
      assert(iu.containsInt(update->lms, 0, update->getSize() - 1, lm));
  }
#endif
  return changed;
}

void LmCausal::printSetOpInput(LmAoNode *nodeToUpdate,
                               const int *nodesToCombine, int size,
                               string setOperation) const {
  cout << endl << "updating ";
  nodeToUpdate->print();
...
bool LmCausal::setToSelfLM(LmAoNode *update) {
  bool changed = !((!update->containsFullSet) && (update->numLMs == 1) &&
                   (update->lms[0] == update->ownIndex));
  if (changed) {
...
void LmCausal::prettyprintAndOrGraph() {
  ...
  for (int i = 0; i < numNodes; i++) {
    for (int j = 0; j < (int)nodes[i]->numAffectedNodes; j++) {
      cout << "    n" << i << " -> "
           << "n" << nodes[i]->affectedNodes[j] << ";" << endl;
    }
  }
...
int LmCausal::getNumLMs() { return this->numLMs; }

landmark **LmCausal::getLMs() { return landmarks; }
...
void LmCausal::iterate() { this->iterI++; }
} // namespace progression
