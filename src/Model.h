...
public:
  ...
  Model();
  // use this constructor only if you want to use the apply and decompose method
  // of the model that perform progression
  Model(bool trackTasksInTN, eMaintainTaskReachability maintainTaskReachability,
        bool progressEffectLess, bool progressOneModActions);
  virtual ~Model();

  // read the model from an input stream
  void read(istream *inputStream);
  // create a search node that contains the initial task -- alternatively used
  // initialTask
  ...
  int *firstIndex; // array for each SAS+ variable, indicates the first fact
                   // that belongs to this variable
  int *lastIndex;  // array for each SAS+ variable, indicates the last fact that
                   // belongs to this variable
  // Note that firstIndex[i] == lastIndex[i] might be possible. If so, this
  // variable is classical STRIPS variable, i.e. it is either true or false. If
  // firstIndex[i] != lastIndex[i], then this is true SAS+ variable, i.e.
  // *exactly* one of the facts is true in each state
  string *varNames;   // array with names of the variables
  int *varOfStateBit; // array for each state-bit, the number of the variable it
                      // belongs to
  ...
  int **ordering; // this is a list of ints (p1,s2, p2,s2, ...) means that p1 is
                  // before s2, p2 before s2, ...
  int *numOrderings; // this is the length of the ARRAY, not the number of
  ...
  int *sccsCyclic = nullptr; // these may be sccs with size one but with a self
                             // loop, or sccs greater than one
...
private:
  ...
  tuple<int *, int *, int **> readConditionalIntList(string s, int &sizeA,
                                                     int &sizeB, int *&sizeC);
...
private:
  ...
  void methodTopSortDFS(int cur, map<int, unordered_set<int>> &adj,
