...
Model::Model(bool trackTasks,
             eMaintainTaskReachability maintainTaskReachability,
             bool progressEffectLess, bool progressOneModActions)
    : trackTasksInTN(trackTasks), progressEffectLess(progressEffectLess),
...
void Model::updateTaskCounterA(searchNode *n, searchNode *parent, int action) {
  int ai = iu.indexOf(parent->containedTasks, 0, parent->numContainedTasks - 1,
                      action);
  ...
  /*
          ...
          cout << parent->containedTasks[i] << "*" <<
     parent->containedTaskCount[i] << " ";
          }
          cout << endl;
          cout << "new ";
...
*/
  ...
  */
  ...
  /*	map<int,int> containedTasksUpdated;
          for (int i = 0; i < n->numContainedTasks; i++)
          containedTasksUpdated[n->containedTasks[i]] =
     n->containedTaskCount[i];
          */
  /*	//for(int i = 0; i < this->numTasks; i++) {
  //if(counted[i] != containedTasksUpdated [i]) {
  cout << "TN n:" << n << " Parent " << parent << endl;
  cout << endl << "Used action: " << " " << action << " " <<
  this->taskNames[action] << endl << endl; for(int j = 0; j < this->numTasks;
  j++) if((counted[j] + containedTasksUpdated [j]) > 0) cout << j << " " <<
  this->taskNames[j] << " " << counted[j] << " " << containedTasksUpdated [j] <<
  endl;
  ...
  //if (this->taskNames[action] ==
  "__method_precondition_m-get_image_data[camera0,high_res,objective1,rover0,waypoint0]")
  exit(1);
...
void Model::updateTaskCounterM(searchNode *n, searchNode *parent, int m) {
  ...
  /*cout << "applied method " << m << " -> "  << decomposedTask[m] << " " << "
    ("; for(int i = 0; i < this->numSubTasks[m]; i++) cout <<
    this->subTasks[m][i] << " "; cout << " aka "; for(int i = 0; i <
    this->numDistinctSTs[m]; i++) cout << this->sortedDistinctSubtasks[m][i] <<
    " "; cout << ")" << endl; cout << "old "; for(int i = 0;i <
    parent->numContainedTasks;i++) { cout << parent->containedTasks[i] << "*" <<
    parent->containedTaskCount[i] << " ";
    }
    cout << endl;*/
  ...
  for (int i = 0; i < this->numDistinctSTs[m]; i++) {
    ...
    int index = iu.indexOf(parent->containedTasks, low,
                           parent->numContainedTasks - 1, st);
    if (index >= 0) {
      low = index + 1;
    } else {
      newElements++;
  ...
  int index = iu.indexOf(parent->containedTasks, 0,
                         parent->numContainedTasks - 1, mAbs);
  assert(index >= 0);
  ...
  bool removeAbs = ((parent->containedTaskCount[index] == 1) &&
                    (iu.indexOf(sortedDistinctSubtasks[m], 0,
                                numDistinctSTs[m] - 1, mAbs) < 0));
  if (removeAbs) {
    n->numContainedTasks--;
  }
  ...
  */
  ...
  //if(counted[i] != containedTasksUpdated [i]) {
  cout << "TN n:" << n << " Parent " << parent << endl;
  cout << endl << "Used method: " << " " << m << " " << this->methodNames[m] <<
  endl << endl; for(int j = 0; j < this->numTasks; j++) if((counted[j] +
  containedTasksUpdated [j]) > 0) cout << j << " " << this->taskNames[j] << " "
  << counted[j] << " " << containedTasksUpdated [j] << endl;
...
searchNode *Model::decompose(searchNode *n, int taskNo, int method) {
  ...
  int mySolutionStepInstanceNumber =
      progression::currentSolutionStepInstanceNumber++;
#endif
  ...
  result->numAbstract = n->numAbstract + numFirstAbstractSubTasks[method] -
                        1; // subtract the decomposed one
...
#ifdef ONEMODMETH
      if (numMethodsForTask[result->unconstraintAbstract[current]->task] == 1)
        oneMod->push(current);
#endif
      current++;
    }
  ...
  // add the successors of the decomposed task as successor of the method's last
  // tasks
  pair<planStep **, planStep **> mInstance =
  ...
  for (int i = 0; i < numLastTasks[method]; i++) {
    ...
    for (int j = 0; j < decomposed->numSuccessors; j++) {
      mInstance.second[i]->successorList[j] = decomposed->successorList[j];
    }
  }
    ...
    if (isPrimitive[mInstance.first[i]->task]) {
      ...
      if (progressEffectLess) {
        if ((this->numAdds[result->unconstraintPrimitive[primI]->task] == 0) &&
            (this->numDels[result->unconstraintPrimitive[primI]->task] == 0) &&
...
#ifdef ONEMODMETH
      if (numMethodsForTask[result->unconstraintAbstract[absI]->task] == 1)
        oneMod->push(absI);
#endif
      absI++;
    }
  ...
  result->solution->parentSolutionStepInstanceNumber =
      decomposed->parentSolutionStepInstanceNumber;
#endif
  ...
  for (int iST = 0; iST < this->numSubTasks[method]; iST++) {
    ...
    if (!iu.containsInt(n->containedTasks, 0, n->numContainedTasks - 1, st)) {
      ...
      if (iLmMapping >= 0) { // check LMs for this task
        for (int iLMM = 0; iLMM < n->lookForT->lookFor[iLmMapping]->size;
             iLMM++) {
          int iLMglobal =
              n->lookForT->lookFor[iLmMapping]->containedInLMs[iLMM];
          ...
          if ((lm->connection == atom) || (lm->connection == disjunctive)) {
            ...
            // more than one task contained in the lm might be in the current
            // method therefore we use the "result" object for the following
            // check
            bool fulfilled = true;
            for (int iCon = 0; iCon < lm->size; iCon++) {
              int tCon = lm->lm[iCon];
              if (!iu.containsInt(result->containedTasks, 0,
                                  result->numContainedTasks - 1, tCon)) {
                fulfilled = false;
                break;
              }
            }
  ...
  if (newlyReachedT.size() > 0) {
    ...
    while (it != needToModify.end()) {
      ...
      while (n->lookForT->lookFor[copyI]->entry < taskToModify) {
        result->lookForT->lookFor[copyI - deleted] =
            n->lookForT->lookFor[copyI];
      ...
      // modify the entry
      result->lookForT->lookFor[copyI - deleted] =
          new LmMap(n->lookForT->lookFor[copyI]->entry,
                    n->lookForT->lookFor[copyI]->size);
      int localDeleted = 0;
      for (int i = 0; i < n->lookForT->lookFor[copyI]->size; i++) {
        if (newlyReachedT.find(
                n->lookForT->lookFor[copyI]->containedInLMs[i]) ==
            newlyReachedT.end()) {
          result->lookForT->lookFor[copyI - deleted]
              ->containedInLMs[i - localDeleted] =
              n->lookForT->lookFor[copyI]->containedInLMs[i];
        } else {
          // cout << "deleted " <<
          // n->lookForT->lookFor[copyI]->containedInLMs[i] << endl;
          localDeleted++;
      ...
      if (it == needToModify.end()) {
        ...
        while (copyI < n->lookForT->size) {
          // cout << "copy " << copyI << " to index " << (copyI - deleted) <<
          // endl;
          result->lookForT->lookFor[copyI - deleted] =
              n->lookForT->lookFor[copyI];
          copyI++;
  ...
  if (progressEffectLess) {
    for (int ac = effectLess->getFirst(); ac >= 0; ac = effectLess->getNext()) {
      assert(((this->numAdds[result->unconstraintPrimitive[ac]->task] == 0) &&
              (this->numDels[result->unconstraintPrimitive[ac]->task] == 0) &&
              (this->isApplicable(result,
                                  result->unconstraintPrimitive[ac]->task))));
  ...
  if (progressOneModActions) {
    if ((result->numAbstract == 0) && (result->numPrimitive > 0)) {
      ...
      if ((applicable != NOACTION) && (applicable != FORBIDDEN)) {
        assert(isApplicable(result,
                            result->unconstraintPrimitive[applicable]->task));
        searchNode *n2 = this->apply(result, applicable);
        delete result;
        result = n2;
        numOneModActions++;
      } else if (applicable == NOACTION) { // there is no abstract task and no
  ...
  for (int t = oneMod->getFirst(); t >= 0; t = oneMod->getNext()) {
    assert(this->numMethodsForTask[result->unconstraintAbstract[t]->task] == 1);
  ...
  *stateSpaceFile << "edge " << n->searchNodeID << " " << result->searchNodeID
                  << endl;
#endif
...
pair<planStep **, planStep **>
Model::initializeMethod(int method
#ifdef TRACESOLUTION
                        ,
...
#ifdef TRACESOLUTION
    stepPointerList[i]->parentSolutionStepInstanceNumber =
        parentSolutionStepIndex;
  ...
  for (int i = 0; i < numOrderings[method]; i += 2) {
    ...
    stepPointerList[pred]
        ->successorList[stepPointerList[pred]->numSuccessors++] =
...
searchNode *Model::apply(searchNode *n, int taskNo) {
  ...
  for (int i = 0; i < n->numPrimitive; i++) {
    if (i != taskNo) {
      for (int j = 0; j < n->unconstraintPrimitive[i]->numSuccessors; j++) {
        potentialPredecessors.push_front(
...
#ifdef ONEMODMETH
    if (numMethodsForTask[result->unconstraintAbstract[currentA]->task] == 1)
  ...
  for (int i = 0; i < n->numPrimitive; i++) {
    if (i != taskNo) {
      result->unconstraintPrimitive[currentP] = n->unconstraintPrimitive[i];
      if (progressEffectLess) {
        if ((this->numAdds[result->unconstraintPrimitive[currentP]->task] ==
            ...
            (this->numDels[result->unconstraintPrimitive[currentP]->task] ==
             0) &&
            (this->isApplicable(
                result, result->unconstraintPrimitive[currentP]->task))) {
  ...
  result->solution->prev = n->solution;
#ifdef TRACESOLUTION
  result->solution->mySolutionStepInstanceNumber =
      progression::currentSolutionStepInstanceNumber++;
  result->solution->myPositionInParent = progressed->myPositionInParent;
  result->solution->parentSolutionStepInstanceNumber =
      progressed->parentSolutionStepInstanceNumber;
#endif
  result->solution->pointersToMe = 1;
  result->modificationDepth = n->modificationDepth + 1;
  result->mixedModificationDepth =
      n->mixedModificationDepth + this->actionCosts[progressed->task];
  ...
  for (int i = 0; i < numAdds[progressed->task]; i++) {
    ...
    if (!n->state[addedF]) {
      ...
      if (iLmMapping >= 0) { // check LMs for this task
        for (int iLMM = 0; iLMM < n->lookForF->lookFor[iLmMapping]->size;
             iLMM++) {
          int iLMglobal =
              n->lookForF->lookFor[iLmMapping]->containedInLMs[iLMM];
          ...
          if ((lm->connection == atom) || (lm->connection == disjunctive)) {
            ...
            // more than one state feature contained in the lm might be set by
            // the current action therefore we use the "result" object for the
            // following check
            bool fulfilled = true;
            for (int iCon = 0; iCon < lm->size; iCon++) {
              int fCon = lm->lm[iCon];
  ...
  if (newlyReachedLM.size() > 0) {
    ...
    while (it != needToModify.end()) {
      ...
      while (n->lookForF->lookFor[copyI]->entry < taskToModify) {
        result->lookForF->lookFor[copyI - deleted] =
            n->lookForF->lookFor[copyI];
      ...
      // modify the entry
      result->lookForF->lookFor[copyI - deleted] =
          new LmMap(n->lookForF->lookFor[copyI]->entry,
                    n->lookForF->lookFor[copyI]->size);
      int localDeleted = 0;
      for (int i = 0; i < n->lookForF->lookFor[copyI]->size; i++) {
        ...
        if (newlyReachedLM.find(lm) == newlyReachedLM.end()) {
          result->lookForF->lookFor[copyI - deleted]
              ->containedInLMs[i - localDeleted] =
              n->lookForF->lookFor[copyI]->containedInLMs[i];
        } else {
          // cout << "deleted " <<
          // n->lookForF->lookFor[copyI]->containedInLMs[i] << endl;
          localDeleted++;
  ...
  while ((!containsOne) && (iEff < this->numAdds[progressed->task])) {
    containsOne = iu.containsInt(n->fLMs, 0, n->numfLMs - 1,
                                 this->addLists[progressed->task][iEff]);
    iEff++;
  }
  if (containsOne) {
    result->fLMs =
        new int[n->numfLMs - 1]; // might be too large, but not too small
    result->numfLMs = 0;
    result->reachedfLMs = n->reachedfLMs;
    for (int iLM = 0; iLM < n->numfLMs; iLM++) {
      int lm = n->fLMs[iLM];
      if (!iu.containsInt(this->addLists[progressed->task], 0,
                          this->numAdds[progressed->task] - 1, lm)) {
    ...
    if (numMethodsForTask[i] > 0) {
      ...
      if (!isPrimitive[i]) {
        cout << "Warning: The task " << taskNames[i]
             << " is abstract but there is no method to decompose it." << endl;
      }
    }
...
void Model::readClassical(istream &domainFile) {
  ...
  *sStream >> numVars;
  ...
  for (int informationType = 0; informationType < 3; informationType++) {
    int &num = (informationType == 0)
                   ? numStrictMutexes
                   : ((informationType == 1) ? numMutexes : numInvariants);
    int *&size = (informationType == 0)
                     ? strictMutexesSize
                     : ((informationType == 1) ? mutexesSize : invariantsSize);
    int **&elems = (informationType == 0)
                       ? strictMutexes
                       : ((informationType == 1) ? mutexes : invariants);
  ...
  *sStream >> numActions;
  ...
  for (int i = 0; i < numActions; i++) {
    ...
    *sStream >> actionCosts[i];
    ...
    getline(domainFile, line);
    std::tuple<int *, int *, int **> adds =
        readConditionalIntList(line, numAdds[i], numConditionalAdds[i],
    ...
    getline(domainFile, line);
    std::tuple<int *, int *, int **> dels =
        readConditionalIntList(line, numDels[i], numConditionalDels[i],
  ...
  for (int i = 0; i < numActions; i++) {
    ...
    if (intSize < numPrecs[i]) {
      cout << "Action #" << i
           << " prec/add/del-definition contains same state feature twice"
           << endl;
#ifndef NDEBUG
      cout << "Original:";
      for (int j = 0; j < numPrecs[i]; j++)
  ...
  *sStream >> numTasks;
  ...
  for (int i = 0; i < numTasks; i++) {
    getline(domainFile, line);
    if (line.size() == 0) {
      cout << "Input promised " << numTasks
           << " tasks, but the list ended after " << i << endl;
      exit(-1);
    }
    sStream = new stringstream(line);
    *sStream >> isAbstract;
...
void Model::readHierarchical(istream &domainFile) {
  ...
  *sStream >> numMethods;
  ...
  for (int i = 0; i < numMethods; i++) {
    getline(domainFile, line);
    if (domainFile.eof()) {
      cout << "Input promised " << numMethods
           << " methods, but input ended after " << i << endl;
      exit(-1);
    }
    methodNames[i] = line;
    getline(domainFile, line);
    ...
    *sStream >> decomposedTask[i];
    ...
    if (ignore.size() < numSubTasks[i]) {
      cout << "Ordering relations of method " << methodNames[i]
           << " are cyclic.";
...
tuple<int *, int *, int **>
Model::readConditionalIntList(string s, int &sizeA, int &sizeB, int *&sizeC) {
  ...
  if (sizeB) {
    ...
    for (int i = 0; i < sizeB; i++) {
      ...
      for (size_t j = 0; j < conds[i].first.size(); j++)
        C[i][j] = conds[i].first[j];
    }
  }
...
int *Model::readIntList(string s, int &size) {
  ...
  while (x != -1) {
    if (x < 0)
      x++; // convert back to -i-1 instead of -i-2
    v.push_back(x);
    sStream >> x;
  }
  size = v.size();
...
void Model::printSummary() {
  ...
  cout << "- Instance is totally-ordered: "
       << ((isTotallyOrdered) ? "yes" : "no") << endl;
  cout << "- Instance has unique paths: " << ((isUniquePaths) ? "yes" : "no")
       << endl;
  cout << "- Instance is parallel sequences: "
       << ((isParallelSequences) ? "yes" : "no") << endl;
}
...
void Model::printMethod(int mI) {
  ...
  for (int i = 0; i < numOrderings[mI]; i += 2) {
    cout << "   " << ordering[mI][i] << " < " << ordering[mI][i + 1] << endl;
  }
  cout << "   First tasks: ";
  ...
  cout << endl;
  cout << "   Number of abstract first tasks: " << numFirstAbstractSubTasks[mI]
       << endl;
...
void Model::calcSCCs() {
  ...
  tarjan(initialTask); // this works only if there is a single initial task and
                       // all tasks are connected to to that task
  ...
  for (std::set<int>::iterator it = selfLoopSccs.begin();
       it != selfLoopSccs.end(); it++) {
    sccsCyclic[j] = *it;
    j++;
  }
...
void Model::tarjan(int v) {
  ...
  for (int iM = 0; iM < numMethodsForTask[v]; iM++) { // iterate methods
    ...
    for (int iST = 0; iST < numSubTasks[m];
         iST++) { // iterate subtasks -> these are the adjacent nodes
...
void Model::constructSCCGraph() {
  ...
  if (numCyclicSccs == 0) {
    ...
    cout << "- Number of cyclic SCCs of size 1: " << numSccOneWithSelfLoops
         << endl;
  }
  ...
  if (maintainTaskReachability != mtrNO) {
    ...
    while (!stack.empty()) {
      ...
      for (int i = 0; i < sccGnumSucc[scc]; i++) { // loop over successor sccs
        ...
        int someTask =
            sccToTasks[nextSCC][0]; // reachability is equal for all tasks in an
                                    // scc -> we need only to consider one
        assert(numReachable[someTask] >=
               0); // should have been set before because it belongs to a
                   // successor scc
      ...
      for (int i = 0; i < sccSize[scc]; i++) {
        ...
        int pos = 0;
        for (set<int>::iterator j = tReachable.begin(); j != tReachable.end();
             j++, pos++) {
...
void Model::calcMinimalImpliedX() {
  ...
  while (!h.empty()) {
    ...
    if (n->isMethod) {
      ...
      minImpliedDistanceM[n->id] =
          1; // a distance of one is implied by the decomposition itself
      ...
      for (int i = 0; i < this->numMethodsForTask[n->id]; i++) {
        ...
        minEstimatedCosts[n->id] =
            min(minEstimatedCostsM[m], minEstimatedCosts[n->id]);
        minImpliedDistance[n->id] =
            min(minImpliedDistanceM[m], minImpliedDistance[n->id]);
      }
  ...
  /*
     for (int i = 0 ; i < this->numTasks; i++) {
     cout << this->taskNames[i] << " c:" << minEstimatedCosts[i] << " d:" <<
...
void Model::writeToPDDL(string dName, string pName) {
  ...
  for (int i = 0; i < numMethods; i++) {
    ...
    if (possibleMethodPrecondition >= 0) {
      ...
      if (allSuccessors.size() != numSubTasks[i])
        possibleMethodPrecondition =
            -1; // not the first task, no not possible to write as precondition
    }
    ...
    dfile << "     :parameters ()" << endl;
    dfile << "     :task ("
          << su.cleanStr(this->taskNames[this->decomposedTask[i]]) << ")"
          << endl;
    if (possibleMethodPrecondition >= 0) {
      ...
      for (int j = 0; j < this->numPrecs[preconditionTask]; j++) {
        dfile << "         ("
              << su.cleanStr(
                     this->factStrs[this->precLists[preconditionTask][j]])
              << ")" << endl;
      }
      dfile << "     )" << endl;
    }
    ...
    if (methodIsTotallyOrdered[i]) {
      ...
      for (int k = 0; k < numSubTasks[i]; k++) {
        ...
        if (j == possibleMethodPrecondition)
          continue;
        ...
        dfile << "        (" << su.cleanStr(taskNames[subTasks[i][j]]) << ")"
              << endl;
      }
      dfile << "     )" << endl;
      ...
      for (int j = 0; j < numSubTasks[i]; j++) {
        ...
        if (subTasks[i][j] < this->numActions)
          actionOccurs[subTasks[i][j]] = true;
        dfile << "        (task" << j << " ("
              << su.cleanStr(taskNames[subTasks[i][j]]) << "))" << endl;
      }
      dfile << "     )" << endl;
      ...
      if (this->numOrderings[i]) {
        ...
        while (j < this->numOrderings[i]) {
          if (this->ordering[i][j] == possibleMethodPrecondition ||
              noopIDs.count(this->ordering[i][j]) ||
              noopIDs.count(this->ordering[i][j] + 1)) {
            j += 2;
            continue;
          }
          if (firstOrdering) {
          ...
          dfile << "        (task" << this->ordering[i][j] << " < task"
                << this->ordering[i][j + 1] << ")" << endl;
          j += 2;
        }
        if (!firstOrdering)
          dfile << "     )" << endl;
  ...
  for (int i = 0; i < this->numActions; i++) {
    if (!actionOccurs[i])
      continue; // don't output method precondition actions.
    ...
    if (this->numPrecs[i]) {
      ...
      for (int j = 0; j < this->numPrecs[i]; j++) {
        dfile << "         ("
              << su.cleanStr(this->factStrs[this->precLists[i][j]]) << ")"
              << endl;
      }
      dfile << "     )" << endl;
    }
    if (this->numAdds[i] + this->numDels[i]) {
      ...
      for (int j = 0; j < this->numAdds[i]; j++) {
        dfile << "         ("
              << su.cleanStr(this->factStrs[this->addLists[i][j]]) << ")"
              << endl;
      }
      for (int j = 0; j < this->numDels[i]; j++) {
        dfile << "         (not("
              << su.cleanStr(this->factStrs[this->delLists[i][j]]) << "))"
              << endl;
      }
      dfile << "     )" << endl;
    }
...
void Model::methodTopSortDFS(int cur, map<int, unordered_set<int>> &adj,
                             map<int, int> &colour, int &curpos, int *order) {
  assert(colour[cur] != 1);
  if (colour[cur])
    return;
  ...
  for (const int &nei : adj[cur])
    methodTopSortDFS(nei, adj, colour, curpos, order);
  colour[cur] = 2;
...
bool Model::isMethodTotallyOrdered(int method) {
  ...
  for (size_t i = 0; i < numSubTasks[method]; i++)
    if (!colour[i])
      methodTopSortDFS(i, adj, colour, curPos, methodTotalOrder[method]);
...
void Model::computeTransitiveChangeOfMethodOrderings(bool closure, int i) {
  ...
  /////// the method is totally ordered iff every task has exactly one successor
  /// and one predecessor except for one task each
  vector<int> ord;
