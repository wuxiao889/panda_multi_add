...
void PDT::resetPruning(Model *htn) {
  ...
  if (expanded) {
    ...
    for (unsigned int at = 0; at < possibleAbstracts.size(); at++)
      prunedMethods[at].assign(htn->numMethodsForTask[possibleAbstracts[at]],
                               false);
  }
  ...
  if (mother != nullptr) {
    ...
    prunedCausesAbstractStart =
        (uint32_t *)calloc(possibleAbstracts.size(), sizeof(uint32_t));
    ...
    prunedCausesForAbstract =
        (uint64_t *)calloc(prunedCausesSize, sizeof(uint64_t));
    for (int i = 0; i < prunedCausesSize; i++)
      prunedCausesForAbstract[i] = 0;

    free(prunedCausesPrimitiveStart);
    free(prunedCausesForPrimitive);
    prunedCausesPrimitiveStart =
        (uint32_t *)calloc(possiblePrimitives.size(), sizeof(uint32_t));
    prunedCausesSize = 0;
...
void PDT::expandPDT(Model *htn, bool effectLessActionsInSeparateLeaf) {
  ...
  for (int tIndex = 0; tIndex < possibleAbstracts.size(); tIndex++) {
    ...
    for (int m = 0; m < htn->numMethodsForTask[t]; m++) {
      applicableMethodsForSOG.push_back(
          make_tuple(htn->taskToMethods[t][m], tIndex, m));
    }
  }
  ...
  sog = optimiseSOG(applicableMethodsForSOG, htn,
                    effectLessActionsInSeparateLeaf);
#ifndef NDEBUG
  ...
  listIndexOfChildrenForMethods =
      (causePointer *)calloc(c, sizeof(causePointer));
  for (int i = 0; i < c; i++)
    listIndexOfChildrenForMethods[i].present = false;
  ...
  vector<map<int, int>> positionOfPrimitiveTasksInChildren(
  ...
  vector<vector<vector<pair<int, int>>>> childCausesForAbstracts(
      sog->numberOfVertices);
  vector<vector<vector<pair<int, int>>>> childCausesForPrimitives(
      sog->numberOfVertices);
    ...
    for (const auto &[t, causes] : childrenTasks[c]) {
      ...
      for (const auto &[tIndex, mIndex] : causes) {
        ...
        if (t < htn->numActions) {
          ...
          childCausesForPrimitives[c].back().push_back(
              make_pair(tIndex, mIndex));
        } else {
          position = childCausesForAbstracts[c].back().size();
          childCausesForAbstracts[c].back().push_back(
              make_pair(tIndex, mIndex));
        }
        ...
        getListIndexOfChildrenForMethods(tIndex, mIndex, c)->isPrimitive =
            t < htn->numActions;
        getListIndexOfChildrenForMethods(tIndex, mIndex, c)->taskIndex =
            subIndex;
        getListIndexOfChildrenForMethods(tIndex, mIndex, c)->causeIndex =
            position;
    ...
    for (size_t pIndex = 0; pIndex < possiblePrimitives.size(); pIndex++) {
      ...
      if (!found) {
        if (effectLessActionsInSeparateLeaf) {
          // childrenTasks[0][p] = _empty; no need to do this, as there are no
          // duplicates in the list of primitives
          ...
          for (size_t c = 0; c < sog->numberOfVertices; c++) {
            if (childrenTasks[c].size() != 0) {
              ...
              bool itEffectless =
                  htn->numAdds[itsFirst] == 0 && htn->numDels[itsFirst] == 0;
              if (itEffectless != iameffectLess)
                continue;
            }
            //
            selectedChild = c;
          ...
          if (!childSelected) {
            cout << "Edge case: no proper child selectable. Not yet "
                    "implemented. mail g.behnke@uva.nl for bugfixing."
                 << endl;
            exit(0);
          }
        } else {
      ...
      positionOfPrimitivesInChildren.push_back(make_tuple(
          selectedChild, positionOfPrimitiveTasksInChildren[selectedChild][p],
          childCausesForPrimitives
              [selectedChild]
  ...
  for (size_t c = 0; c < sog->numberOfVertices; c++) {
    ...
    children[c]->numberOfCausesPerAbstract = (uint32_t *)calloc(
        children[c]->possibleAbstracts.size(), sizeof(uint32_t));
    children[c]->startOfCausesPerAbstract = (uint32_t *)calloc(
        children[c]->possibleAbstracts.size(), sizeof(uint32_t));
    int currentPosition = 0;
    for (int at = 0; at < children[c]->possibleAbstracts.size(); at++) {
      children[c]->startOfCausesPerAbstract[at] = currentPosition;
      children[c]->numberOfCausesPerAbstract[at] =
          childCausesForAbstracts[c][at].size();
      currentPosition += children[c]->numberOfCausesPerAbstract[at];
    }

    children[c]->causesForAbstracts =
        (taskCause *)calloc(currentPosition, sizeof(taskCause));
    for (int at = 0; at < children[c]->possibleAbstracts.size(); at++) {
      for (int cause = 0; cause < children[c]->numberOfCausesPerAbstract[at];
           cause++) {
        children[c]->getCauseForAbstract(at, cause)->taskIndex =
            childCausesForAbstracts[c][at][cause].first;
        children[c]->getCauseForAbstract(at, cause)->methodIndex =
            childCausesForAbstracts[c][at][cause].second;
      }
    }

    // and once for the primitives

    children[c]->numberOfCausesPerPrimitive = (uint32_t *)calloc(
        children[c]->possiblePrimitives.size(), sizeof(uint32_t));
    children[c]->startOfCausesPerPrimitive = (uint32_t *)calloc(
        children[c]->possiblePrimitives.size(), sizeof(uint32_t));
    currentPosition = 0;
    for (int at = 0; at < children[c]->possiblePrimitives.size(); at++) {
      children[c]->startOfCausesPerPrimitive[at] = currentPosition;
      children[c]->numberOfCausesPerPrimitive[at] =
          childCausesForPrimitives[c][at].size();
      currentPosition += children[c]->numberOfCausesPerPrimitive[at];
    }

    children[c]->causesForPrimitives =
        (taskCause *)calloc(currentPosition, sizeof(taskCause));
    for (int at = 0; at < children[c]->possiblePrimitives.size(); at++) {
      for (int cause = 0; cause < children[c]->numberOfCausesPerPrimitive[at];
           cause++) {
        children[c]->getCauseForPrimitive(at, cause)->taskIndex =
            childCausesForPrimitives[c][at][cause].first;
        children[c]->getCauseForPrimitive(at, cause)->methodIndex =
            childCausesForPrimitives[c][at][cause].second;
      }
    }
  }
  ...
  // cout << "\t\t\t\t" << listIndexOfChildrenForMethodsEntries * (sizeof(int)*3
  // + sizeof(bool)) / 1024 << endl;
#endif
  ...
  cout << "SOG " << setw(8) << setprecision(3) << prep_in_ms << " " << setw(8)
       << setprecision(3) << sog_in_ms << " " << setw(8) << setprecision(3)
       << after_in_ms << " ms" << endl;
#endif
...
void printPDT(Model *htn, PDT *cur, int indent) {
  printIndentMark(indent, 10, cout);
  cout << "Node: " << path_string(cur->path) << endl;
  for (size_t p = 0; p < cur->possiblePrimitives.size(); p++) {
    int prim = cur->possiblePrimitives[p];
    printIndentMark(indent + 5, 10, cout);
    cout << color(cur->prunedPrimitives[p] ? Color::RED : Color::WHITE,
                  "p " + htn->taskNames[prim])
         << endl;
  }
  for (size_t a = 0; a < cur->possibleAbstracts.size(); a++) {
    int abs = cur->possibleAbstracts[a];
    printIndentMark(indent + 5, 10, cout);
    cout << color(cur->prunedAbstracts[a] ? Color::RED : Color::WHITE,
                  "a " + htn->taskNames[abs])
         << endl;
    if (cur->expanded) {
      for (size_t m = 0; m < htn->numMethodsForTask[cur->possibleAbstracts[a]];
           m++) {
        printIndentMark(indent + 10, 10, cout);
        cout << color(cur->prunedMethods[a][m] ? Color::RED : Color::WHITE,
                      "m " + htn->methodNames[htn->taskToMethods[abs][m]])
             << endl;
        // for (size_t s = 0; s < htn->numSubTasks[htn->taskToMethods[abs][m]];
        // s++){ 	printIndentMark(indent + 15, 10, cout); 	cout <<
        // htn->taskNames[htn->subTasks[htn->taskToMethods[abs][m]][s]] << endl;
        // }
      }
    }
    /*if (cur->mother != nullptr){
            for (size_t c = 0; c < cur->causesForAbstracts[a].size(); c++){
                    printIndentMark(indent + 10, 10, cout);
                    cout << color(cur->prunedCausesForAbstract[a][c]?Color::RED
    : Color::WHITE, "c ") << endl;
            }
    }*/
  }
...
void PDT::printDot(Model *htn, ofstream &dfile) {
  ...
  */
  dfile << "\ta" << path_string_no_sep(path) << "[label=\"" << s << "\"]"
        << endl;
  for (PDT *child : children) {
    dfile << "\ta" << path_string_no_sep(path) << " -> a"
          << path_string_no_sep(child->path) << endl;
    child->printDot(htn, dfile);
  }
}

void printPDT(Model *htn, PDT *cur) { printPDT(htn, cur, 0); }
...
void PDT::assignVariableIDs(sat_capsule &capsule, Model *htn) {
  ...
  if (!vertexVariables) {
    ...
    DEBUG(string name = "none          @ " + pad_path(path);
  ...
  for (size_t p = 0; p < possiblePrimitives.size(); p++) {
    ...
    DEBUG(string name = "prim     " + pad_int(possiblePrimitives[p]) + " @ " +
                        pad_path(path) + ": " +
                        pad_string(htn->taskNames[possiblePrimitives[p]]);
  ...
  for (size_t a = 0; a < possibleAbstracts.size(); a++) {
    ...
    DEBUG(string name = "abstract " + pad_int(possibleAbstracts[a]) + " @ " +
                        pad_path(path) + ": " +
                        pad_string(htn->taskNames[possibleAbstracts[a]]);
          capsule.registerVariable(num, name););
  }
  ...
  if (!expanded)
    return; // if I am not expanded, I have no decomposition clauses

  methodVariablesStartIndex =
      (uint32_t *)calloc(possibleAbstracts.size(), sizeof(uint32_t));
  int methodVars = 0;
  for (size_t a = 0; a < possibleAbstracts.size(); a++) {
  ...
  for (int i = 0; i < methodVars; i++)
    methodVariables[i] = -1;

  for (size_t a = 0; a < possibleAbstracts.size(); a++) {
    for (size_t mi = 0; mi < htn->numMethodsForTask[possibleAbstracts[a]];
         mi++) {
#ifdef NO_PRUNED_VARIABLES
      if (prunedMethods[a][mi])
        continue;
#endif
      // don't generate variables pertaining to children (and methods) twice
      if (*getMethodVariable(a, mi) != -1)
        continue;

      int num = capsule.new_variable();
      *getMethodVariable(a, mi) = num;

      DEBUG(int &t = possibleAbstracts[a]; int m = htn->taskToMethods[t][mi];
            string name = "method   " + pad_int(m) + " @ " + pad_path(path) +
                          ": " + pad_string(htn->methodNames[m]);
            capsule.registerVariable(num, name););
    }
  }
...
void PDT::assignOutputNumbers(void *solver, int &currentID, Model *htn) {
  ...
  for (size_t aIndex = 0; aIndex < possibleAbstracts.size(); aIndex++) {
    ...
    if (ipasir_val(solver, abs) > 0) {
      ...
      for (size_t mIndex = 0;
           mIndex < htn->numMethodsForTask[possibleAbstracts[aIndex]];
           mIndex++) {
  ...
  for (PDT *&child : children)
    child->assignOutputNumbers(solver, currentID, htn);
}
...
void PDT::propagatePruning(Model *htn) {
  ...
  if (mother != nullptr) {
    // check whether it might be impossible for a primitive or abstract to be
    // caused This is where information flows from the top to the bottom
    for (size_t p = 0; p < possiblePrimitives.size(); p++) {
      if (prunedPrimitives[p])
        continue;
  ...
  for (size_t a = 0; a < possibleAbstracts.size(); a++)
    if (prunedAbstracts[a]) {
      if (expanded)
        for (size_t m = 0; m < htn->numMethodsForTask[possibleAbstracts[a]];
             m++) {
#ifndef NDEBUG
          cout << "  M  " << a << " " << m << endl;
#endif
  ...
  if (children.size()) {
    for (size_t p = 0; p < possiblePrimitives.size(); p++)
      if (prunedPrimitives[p]) {
        ...
        if (!children[childIndex]->getPrunedCausesForPrimitive(
                childPrimIndex, childCauseIndex)) {
#ifndef NDEBUG
          cout << "  SS CC " << childIndex << " " << childPrimIndex << " "
               << childCauseIndex << endl;
#endif
          childrenChanged[childIndex] = true;
          children[childIndex]->setPrunedCausesForPrimitive(childPrimIndex,
                                                            childCauseIndex);
        }
      }
    ...
    for (size_t a = 0; a < possibleAbstracts.size(); a++)
      for (size_t m = 0; m < htn->numMethodsForTask[possibleAbstracts[a]];
           m++) {
        if (!prunedMethods[a][m])
          continue;

        for (size_t childIndex = 0; childIndex < children.size();
             childIndex++) {
          if (!getListIndexOfChildrenForMethods(a, m, childIndex)->present)
            continue;
          bool isPrimitive =
              getListIndexOfChildrenForMethods(a, m, childIndex)->isPrimitive;
          int childTaskIndex =
              getListIndexOfChildrenForMethods(a, m, childIndex)->taskIndex;
          int childCauseIndex =
              getListIndexOfChildrenForMethods(a, m, childIndex)->causeIndex;
          bool causePruned =
              (isPrimitive) ? children[childIndex]->getPrunedCausesForPrimitive(
                                  childTaskIndex, childCauseIndex)
                            : children[childIndex]->getPrunedCausesForAbstract(
                                  childTaskIndex, childCauseIndex);

          // cout << "  SS PCC " << childIndex << " " << childTaskIndex << " "
          // << childCauseIndex << " " << causePruned <<" " << isPrimitive <<
          // endl;
          if (!causePruned) {
            if (isPrimitive)
              children[childIndex]->setPrunedCausesForPrimitive(
                  childTaskIndex, childCauseIndex);
            else
              children[childIndex]->setPrunedCausesForAbstract(childTaskIndex,
                                                               childCauseIndex);
            childrenChanged[childIndex] = true;
#ifndef NDEBUG
            cout << "  SS PCC " << childIndex << " " << childTaskIndex << " "
...
void PDT::countPruning(int &overallSize, int &overallPruning,
  ...
  if (!onlyPrimitives)
    overallSize += possibleAbstracts.size();
...
void PDT::addDecompositionClauses(void *solver, sat_capsule &capsule,
  ...
  // at most one method
  // TODO tested AMO per AT. This gives a lot more clauses ... (performance test
  // is still open)
  ...
  for (size_t p = 0; p < possiblePrimitives.size(); p++) {
    ...
    implies(solver, primitiveVariable[p],
            children[child]->primitiveVariable[pIndex]);
    ...
    for (size_t mi = 0; mi < htn->numMethodsForTask[possibleAbstracts[a]];
      ...
      for (size_t child = 0; child < children.size(); child++) {
        if (getListIndexOfChildrenForMethods(a, mi, child)->present) {
          bool isPrimitive =
              getListIndexOfChildrenForMethods(a, mi, child)->isPrimitive;
          int listIndex =
              getListIndexOfChildrenForMethods(a, mi, child)->taskIndex;
          ...
          // TODO: try out the explicit encoding (m -> -task@child) for all
          // children/tasks [this should lead to a larger encoding ..]
  ...
  for (PDT *&child : children) {
    ...
    for (size_t p = 0; p < child->possiblePrimitives.size(); p++) {
      if (child->primitiveVariable[p] == -1)
        continue;

      vector<int> allCauses;

      for (size_t i = 0; i < child->numberOfCausesPerPrimitive[p]; i++) {
        if (child->getPrunedCausesForPrimitive(p, i))
          continue;
...
void PDT::setPrunedCausesForAbstract(int a, int b) {
  ...
  prunedCausesForAbstract[num] =
      prunedCausesForAbstract[num] | (uint64_t(1) << bit);
}

bool PDT::areAllCausesPrunedAbstract(int a) {
  for (int c = 0; c < numberOfCausesPerAbstract[a]; c++)
    if (!getPrunedCausesForAbstract(a, c))
      return false;
  return true;
}
...
void PDT::setPrunedCausesForPrimitive(int a, int b) {
  ...
  prunedCausesForPrimitive[num] =
      prunedCausesForPrimitive[num] | (uint64_t(1) << bit);
}

bool PDT::areAllCausesPrunedPrimitive(int a) {
  for (int c = 0; c < numberOfCausesPerPrimitive[a]; c++)
    if (!getPrunedCausesForPrimitive(a, c))
      return false;
  return true;
}
