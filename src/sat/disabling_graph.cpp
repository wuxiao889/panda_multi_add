...
void graph::tarjan(int v) {
  ...
  for (int i = 0; i < adjSize[v];
       i++) { // iterate subtasks -> these are the adjacent nodes
    int v2 = adj[v][i];
    if (U[v2]) {
      tarjan(v2);
...
string graph::dot_string(map<int, string> names, map<int, string> nodestyles) {
  ...
  for (int i = 0; i < numVertices; i++) {
    for (int jI = 0; jI < adjSize[i]; jI++) {
      int j = adj[i][jI];
      result +=
          "\ta" + to_string(i) + " -> a" + to_string(j) + " [label=\"\"];\n";
    }
  }
...
bool are_actions_applicable_in_the_same_state(Model *htn, int a, int b) {
  ...
  // checking this is ok for the DG as applying both actions will lead to an
  // inconsistent state
  //
...
graph *compute_disabling_graph(Model *htn, bool no_invariant_inference) {
  ...
  for (int f = 0; f < htn->numStateBits; f++) {
    for (int deletingIndex = 0; deletingIndex < htn->delToActionSize[f];
         deletingIndex++) {
      int deletingAction = htn->delToAction[f][deletingIndex];

      for (int needingIndex = 0; needingIndex < htn->precToActionSize[f];
           needingIndex++) {
        int needingAction = htn->precToAction[f][needingIndex];
        if (deletingAction == needingAction)
          continue; // action cannot disable itself
        if (!no_invariant_inference &&
            !are_actions_applicable_in_the_same_state(htn, deletingAction,
                                                      needingAction))
          continue;
        DEBUG(cout << "DEL NEED: " << deletingAction << " "
                   << htn->taskNames[deletingAction];
              cout << " vs " << needingAction << " "
                   << htn->taskNames[needingAction] << endl;);
    ...
    if (no_invariant_inference) {
      for (int addingIndex = 0; addingIndex < htn->addToActionSize[f];
           addingIndex++) {
        ...
        for (int needingIndex = 0; needingIndex < htn->precToActionSize[f];
             needingIndex++) {
          int needingAction = htn->precToAction[f][needingIndex];
          if (addingAction == needingAction)
            continue; // action cannot disable itself
          if (!no_invariant_inference &&
              !are_actions_applicable_in_the_same_state(htn, addingAction,
                                                        needingAction))
            continue;
          DEBUG(cout << "NEED ADD: " << needingAction << " "
                     << htn->taskNames[needingAction];
                cout << " vs " << addingAction << " "
                     << htn->taskNames[addingAction] << endl;);
      ...
      for (int addingIndex = 0; addingIndex < htn->addToActionSize[f];
        ...
        for (int deletingIndex = 0; deletingIndex < htn->delToActionSize[f];
             deletingIndex++) {
          int deletingAction = htn->delToAction[f][deletingIndex];
          if (addingAction == deletingAction)
            continue; // action cannot disable itself
          if (!no_invariant_inference &&
              !are_actions_applicable_in_the_same_state(htn, addingAction,
                                                        deletingAction))
            continue;
          DEBUG(cout << "DEL-ADD " << deletingAction << " "
                     << htn->taskNames[deletingAction];
                cout << " vs " << addingAction << " "
                     << htn->taskNames[addingAction] << endl;);
...
bool doActionsInterfere(Model *htn, vector<int> &previous, vector<int> &next) {
  ...
  // for (int n : next)
  //	if (prevSet.count(n)) return true; // two identical actions may not be
  ...
  for (const int &p : previous) {
    ...
    for (size_t d = 0; d < htn->numDels[p]; d++)
      if (nextPreconditions.count(htn->delLists[p][d]) ||
          nextAdds.count(htn->delLists[p][d]))
        return true;
  }
  return false;
}

vector<vector<int>> compute_block_compression(Model *htn,
                                              vector<PDT *> &leafs) {
...
// vector<vector<int>> compute_block_compression(Model * htn, graph * dg,
// vector<PDT*> & leafs){ 	vector<vector<int>> blocks;
//
//	vector<int> currentBlock;
//	vector<int> currentPrimitives;
//
