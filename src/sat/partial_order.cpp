...
void generate_matching_formula(void *solver, sat_capsule &capsule, Model *htn,
  ...
  for (int l = 0; l < leafSOG->numberOfVertices; l++) {
    for (int p = 0; p < vars.size(); p++) {
      ...
      DEBUG(capsule.registerVariable(
          matchVar, "match leaf " + pad_int(l) + " + position " + pad_int(p)));
  ...
  for (int l = 0; l < leafSOG->numberOfVertices; l++) {
    ...
    for (int prim = 0; prim < leaf->possiblePrimitives.size(); prim++) {
      if (leaf->primitiveVariable[prim] == -1)
        continue; // pruned
  ...
  for (int p = 0; p < vars.size(); p++) {
    ...
    for (int l = 0; l < leafSOG->numberOfVertices; l++) {
      ...
      for (int primC = 0; primC < leaf->possiblePrimitives.size(); primC++) {
        if (leaf->primitiveVariable[primC] == -1)
          continue; // pruned
        int prim = leaf->possiblePrimitives[primC];

        // if this leaf is connected then the task must be present

        impliesAnd(solver, matching.matchingPerPosition[p][l],
                   leaf->primitiveVariable[primC], variablesPerPrimitive[prim]);
      }
    }
  }
  ...
  for (int l = 0; l < leafSOG->numberOfVertices; l++) {
    ...
    PDT *leaf = leafSOG->leafOfNode[l];
    for (int primC = 0; primC < leaf->possiblePrimitives.size(); primC++) {
      leafPrimitives[leaf->possiblePrimitives[primC]] =
          leaf->primitiveVariable[primC];
    }
      ...
      for (auto [pvar, prim] : vars[p]) {
        if (htn->numAdds[prim] != 0 || htn->numDels[prim] != 0) {
          if (leafPrimitives[prim] > 0) {
            ...
            impliesAnd(solver, matching.matchingPerPosition[p][l], pvar,
                       leafPrimitives[prim]);
          } else {
  ...
  for (int l = 0; l < leafSOG->numberOfVertices; l++) {
    for (int p = 0; p < vars.size(); p++) {
      ...
      DEBUG(capsule.registerVariable(forbiddenVar,
                                     "forbidden leaf " + pad_int(l) +
                                         " + position " + pad_int(p)));
  ...
  for (int l = 0; l < leafSOG->numberOfVertices; l++) {
    for (int lSucc : leafSOG->adj[l]) {
      for (int p = 1; p < vars.size(); p++) {
        implies(solver, matching.matchingPerLeaf[l][p],
                forbiddenPerLeaf[lSucc][p - 1]);
      }
    }
  }
  ...
  for (int l = 0; l < leafSOG->numberOfVertices; l++) {
    for (int p = 0; p < vars.size(); p++) {
      impliesNot(solver, forbiddenPerLeaf[l][p],
                 matching.matchingPerLeaf[l][p]);
    }
  }
