...
void generate_state_transition_formula(
    void *solver, sat_capsule &capsule,
    vector<vector<pair<int, int>>> &actionVariables,
    vector<int> &block_base_variables, Model *htn) {
  ...
  return generate_state_transition_formula(solver, capsule, actionVariables,
...
void generate_state_transition_formula(
    void *solver, sat_capsule &capsule,
    vector<vector<pair<int, int>>> &actionVariables,
    vector<int> &block_base_variables, vector<vector<int>> &blocks,
    Model *htn) {

  cout << "Generating state formula for " << blocks.size() << " blocks."
       << endl;
  ...
  for (size_t time = 0; time <= timesteps; time++) {
    ...
    if (time < timesteps)
      block_base_variables[time] =
          base; // save the variables in the first leaf of the block
    else
      goalBase = base;

    DEBUG(capsule.registerVariable(base, "state var " + pad_int(0) + " @ " +
                                             pad_int(time) + ": " +
                                             pad_string(htn->factStrs[0])));
    ...
    for (size_t svar = 1; svar < htn->numStateBits; svar++) {
      ...
      assert(_r == base + svar);
      DEBUG(capsule.registerVariable(
          base + svar, "state var " + pad_int(svar) + " @ " + pad_int(time) +
                           ": " + pad_string(htn->factStrs[svar])));
    }
  }
  ...
  for (size_t time = 0; time < timesteps; time++) {
    ...
    int nextTimeBase =
        (time == (timesteps - 1)) ? goalBase : block_base_variables[time + 1];
    ...
    for (const int &l : blocks[time]) {
      ...
      for (const auto &[varID, taskID] : actionVariables[l]) {
        ...
        for (size_t precIndex = 0; precIndex < htn->numPrecs[taskID];
             precIndex++)
          implies(solver, varID,
                  thisTimeBase + htn->precLists[taskID][precIndex]);
        // add effects
    ...
    for (size_t svar = 0; svar < htn->numStateBits; svar++) {
      impliesPosAndNegImpliesOr(solver, nextTimeBase + svar,
                                thisTimeBase + svar, causingPositive[svar]);
      impliesPosAndNegImpliesOr(solver, thisTimeBase + svar,
                                nextTimeBase + svar, causingNegative[svar]);
    }
#ifndef NDEBUG
    int frame = get_number_of_clauses();
    cout << setw(4) << time << " actions " << setw(7) << act << " " << setw(8)
         << eff - bef << " " << setw(8) << frame - eff << endl;
#endif

    int afterClauses = get_number_of_clauses();
    // cout << "Timestep clauses " << afterClauses - beforeClauses << " from "
    // << blocks[time].size() << " actions." << endl;
  }
...
void generate_mutex_formula(void *solver, sat_capsule &capsule,
...
void generate_mutex_formula(void *solver, sat_capsule &capsule,
  ...
  for (size_t time = 0; time < blocks.size(); time++) {
    ...
    for (int v = 0; v < htn->numVars; v++) {
      if (htn->firstIndex[v] == htn->lastIndex[v])
        continue; // STRIPS
    ...
    for (int i = 0; i < 2 * htn->numStateBits; i++) {
      ...
      for (const int &b : binary_invariants[i]) {
        ...
        vector<int> vars;
        if (a < 0)
          vars.push_back(-(timeBase + -a - 1));
        else
          vars.push_back(timeBase + a);
        if (b < 0)
          vars.push_back(-(timeBase + -b - 1));
        else
          vars.push_back(timeBase + b);
        atLeastOne(solver, capsule, vars);
      }
  ...
  cout << " SM: " << htn->numStrictMutexes << " M: " << htn->numMutexes
       << " I: " << htn->numInvariants << " SI: " << count_invariants(htn)
       << endl;
}
...
void get_linear_state_atoms(sat_capsule &capsule, vector<PDT *> &leafs,
                            vector<vector<pair<int, int>>> &ret) {
  // these are just the primitives in the correct order

  for (PDT *&leaf : leafs) {
#ifndef NDEBUG
    std::cout << "Position: " << ret.size() << " Leaf " << pad_path(leaf->path)
              << std::endl;
#endif
    ...
    for (size_t p = 0; p < leaf->possiblePrimitives.size(); p++)
      if (leaf->primitiveVariable[p] != -1)
        atoms.push_back(
            make_pair(leaf->primitiveVariable[p], leaf->possiblePrimitives[p]));
...
void get_partial_state_atoms(sat_capsule &capsule, Model *htn, SOG *sog,
                             vector<vector<pair<int, int>>> &ret,
                             bool effectLessActionsInSeparateLeafs) {
  ...
  for (int t = 0; t < sog->numberOfVertices; t++) {
    ...
    if (effectLessActionsInSeparateLeafs && actualAction && effectLessAction)
      exit(0);
    if (!actualAction)
      leafsWithoutActualActions++;
  ...
  cout << "Total Leafs: " << sog->numberOfVertices
       << " of that without effect-actions " << leafsWithoutActualActions
       << " remaining: " << numberOfTimeSteps << endl;
...
#ifndef NDEBUG
    std::cout << "Position: " << t << " succ: " << numSucc
              << " prec: " << numPrec;
    std::cout << "\t\tfrom " << firstPossible << " to " << lastPossible
              << std::endl;
#endif
  ...
  /*	for (int t = 0; t < sog->numberOfVertices; t++){
                  ...
                  for (size_t p = 0; p < htn->numActions; p++){
                          ...
                          DEBUG(capsule.registerVariable(pvar,"action var " +
     pad_int(p) + " @ " + pad_int(t) + ": " + pad_string(htn->taskNames[p])));
  ...
  */
  for (int t = 0; t < numberOfTimeSteps; t++) {
    ...
    for (int p : actionsPerPosition[t]) {
      int pvar = capsule.new_variable();
      DEBUG(capsule.registerVariable(pvar, "action var " + pad_int(p) + " @ " +
                                               pad_int(t) + ": " +
                                               pad_string(htn->taskNames[p])));
      atoms.push_back(make_pair(pvar, p));
    }
