...
sat_capsule::sat_capsule() { number_of_variables = 0; }

int sat_capsule::new_variable() { return ++number_of_variables; }
...
void reset_number_of_clauses() { number_of_clauses = 0; }

int get_number_of_clauses() { return number_of_clauses; }
...
void impliesPosAndNegImpliesOr(void *solver, int i, int j,
                               std::vector<int> &k) {
  ipasir_add(solver, -i);
  ipasir_add(solver, j);
  for (int &x : k)
...
void atMostOneBinomial(void *solver, sat_capsule &capsule,
                       std::vector<int> &is) {
