...
SOG *runPOSOGOptimiser(SOG *sog,
                       vector<tuple<uint32_t, uint32_t, uint32_t>> &methods,
                       Model *htn, bool effectLessActionsInSeparateLeaf) {
  ...
  for (size_t i = 0; i < sog->numberOfVertices; i++) {
    ...
    if (task < htn->numActions && htn->numAdds[task] == 0 &&
        htn->numDels[task] == 0)
      vertices_for_effectless_actions.insert(i);
  }
  sog->methodSubTasksToVertices.push_back(mapping);

  // add ordering
  for (int ordering = 0; ordering < htn->numOrderings[m0]; ordering += 2) {
    sog->adj[htn->ordering[m0][ordering]].insert(
        htn->ordering[m0][ordering + 1]);
    sog->bdj[htn->ordering[m0][ordering + 1]].insert(
        htn->ordering[m0][ordering]);
  }
  ...
  for (size_t mID = 1; mID < methods.size(); mID++) {
    ...
    for (size_t stID = 0; stID < htn->numSubTasks[m]; stID++) {
      int subTask =
          htn->methodTotalOrder[m][stID]; // use positions in topological order,
                                          // may lead to better results

      int myTask = htn->subTasks[m][subTask];
      bool isEffectless = false;
      if (myTask < htn->numActions &&
          (htn->numAdds[myTask] == 0 && htn->numDels[myTask] == 0))
        isEffectless = true;

      // try to find a matching task
      vector<int> matchingVertices;
      for (size_t v = 0; v < sog->numberOfVertices; v++) {
        if (takenVertices.count(v))
          continue;
        // if enabled, effect-less actions may only be grouped together with
        // other effect-less actions
        if (effectLessActionsInSeparateLeaf &&
            isEffectless != vertices_for_effectless_actions.count(v))
          continue;
        ...
        for (size_t ostID = 0; ostID < stID; ostID++) {
          ...
          if (htn->methodSubTasksSuccessors[m][subTask].count(otherSubTask)) {
            if (sog->bdj[v].count(ov))
              goto next_vertex;
            if (antiAdj[v].count(ov))
              goto next_vertex;
          } else if (htn->methodSubTasksPredecessors[m][subTask].count(
                         otherSubTask)) {

            if (sog->adj[v].count(ov))
              goto next_vertex;
            if (antiBdj[v].count(ov))
              goto next_vertex;
          } else {
...
#endif
      // TODO better do this with DP? At least in the total order case, this
      // should be possible
      ...
      if (v == -1)
        v = matchingVertices[0]; // just take the first vertex that fits
      ...
      for (int x : sog->labels[v])
        cout << "\t" << htn->taskNames[x] << " "
             << (x < htn->numActions &&
                 (htn->numAdds[x] == 0 && htn->numDels[x] == 0))
             << endl,
            types.insert((x < htn->numActions &&
                          (htn->numAdds[x] == 0 && htn->numDels[x] == 0)));
      cout << "++++" << htn->taskNames[myTask] << " "
           << (myTask < htn->numActions &&
               (htn->numAdds[myTask] == 0 && htn->numDels[myTask] == 0))
           << endl;
      cout << "-----------------------------------" << endl;
      if (types.size() && types.count(myTask < htn->numActions &&
                                      (htn->numAdds[myTask] == 0 &&
                                       htn->numDels[myTask] == 0)) == 0)
        exit(0);
#endif
      ...
      if (myTask < htn->numActions &&
          (htn->numAdds[myTask] == 0 && htn->numDels[myTask] == 0))
        vertices_for_effectless_actions.insert(v);
...
void addSequenceToSOG(SOG *sog, vector<int> &sequence,
                      vector<int> &matchingPositions, int sequenceID, int begin,
                      int end) {
  int targetLength = end - begin;
  // cout << "OPTI " << end << " - " << begin << " = " << targetLength << endl;

  // (x,y) -> minimum labels needed when putting the first x tasks to the first
  // y positions
  ...
  for (size_t x = 1; x <= sequence.size(); x++) {
    ...
    for (size_t y = 1; y <= targetLength; y++) {
      if (x > y)
        minAdditionalLabels[x][y] = 1000 * 1000; // impossible
      else {
        int labelNeeded = 1 - sog->labels[begin + y - 1].count(task);
        // put label here
        minAdditionalLabels[x][y] =
            labelNeeded + minAdditionalLabels[x - 1][y - 1];
        optimalPutHere[x][y] = true;

        // cout << "COMP " << x << " " << y << " " << labelNeeded << " " ;
        // cout << minAdditionalLabels[x][y-1] << " < " <<
        // minAdditionalLabels[x][y] << endl;
  ...
  while (x != 0 && y != 0) {
    if (optimalPutHere[x][y]) {
      ...
      sog->methodSubTasksToVertices[sequenceID][matchingPositions[x - 1]] =
          begin + y - 1;
      x--;
      y--;
    } else {
...
SOG *linearLabelSetOptimisation(Model *htn,
                                vector<pair<int, vector<int>>> &labelSequence) {
  // sort by number of labels
  sort(labelSequence.begin(), labelSequence.end(),
       [&](pair<int, vector<int>> m1, pair<int, vector<int>> m2) {
  ...
  for (size_t lIndex = 0; lIndex < labelSequence.size(); lIndex++) {
    ...
    // cout << "Label " << labelSequence[lIndex].first << " " << sequence.size()
    // << endl;
    vector<int> indices;
    for (size_t i = 0; i < sequence.size(); i++)
      indices.push_back(i);
    // create the matching
    sog->methodSubTasksToVertices[labelSequence[lIndex].first].resize(
        sequence.size());

    addSequenceToSOG(sog, sequence, indices, labelSequence[lIndex].first, 0,
                     sog->numberOfVertices);
  }
...
void addCompactSequenceToSOG(SOG *sog, vector<int> &sequence,
                             vector<int> &matchingPositions, int sequenceID,
                             Model *htn,
  ...
  for (size_t pos = 0; pos < sequence.size(); pos++) {
    ...
    bool isEffectlessAction =
        task < htn->numActions &&
        (htn->numAdds[task] == 0 && htn->numDels[task] == 0);
    for (size_t v = lastMacthing + 1; v < sog->numberOfVertices; v++) {
      if (isEffectlessAction != effectlessVertices.count(v))
        continue;
    ...
    if (!foundMatching) {
      ...
      sog->methodSubTasksToVertices[sequenceID][matchingPositions[pos]] =
          sog->numberOfVertices - 1;
      if (isEffectlessAction)
        effectlessVertices.insert(sog->numberOfVertices - 1);
    }
  }
}

SOG *runTOSOGOptimiser(SOG *sog,
                       vector<tuple<uint32_t, uint32_t, uint32_t>> &methods,
                       Model *htn, bool effectLessActionsInSeparateLeafs) {
  // cout << endl << endl << endl <<
  // "================================================" << endl;
  ...
  for (size_t mID = 0; mID < methods.size(); mID++) {
    ...
    if (!effectLessActionsInSeparateLeafs)
      addSequenceToSOG(sog, taskSequence, indexSequence, mID, 0,
                       sog->numberOfVertices);
    else
      addCompactSequenceToSOG(sog, taskSequence, indexSequence, mID, htn,
                              effectlessVertices);
  }
...
SOG *runTOSOGOptimiserRecursive(
    SOG *sog, vector<tuple<uint32_t, uint32_t, uint32_t>> &methods,
    Model *htn) {
  // cout << endl << endl << endl <<
  // "================================================" << endl;
  ...
  for (size_t mID = 0; mID < methods.size(); mID++) {
    ...
    for (size_t x = 0; x < htn->numSubTasks[m]; x++)
      if (!htn->sccIsAcyclic
               [htn->taskToSCC[htn->subTasks[m][htn->methodTotalOrder[m][x]]]])
  ...
  for (size_t mID = 0; mID < methods.size(); mID++) {
    ...
    for (size_t x = 0; x < htn->numSubTasks[m]; x++)
      if (!htn->sccIsAcyclic
               [htn->taskToSCC[htn->subTasks[m]
                                            [htn->methodTotalOrder[m][x]]]]) {
        int positionOfThisTaskInSOG =
            recSOG->methodSubTasksToVertices[mID][numRec];
        int numberOfTasksSinceLastRecursive = x - lastRecursive - 1;

        // memorise how much space we need
        spaceNeededInBlocks[mID][positionOfThisTaskInSOG].first =
            lastRecursivePosition;
        spaceNeededInBlocks[mID][positionOfThisTaskInSOG].second =
            numberOfTasksSinceLastRecursive;
    ...
    spaceNeededInBlocks[mID][recSOG->numberOfVertices].first =
        lastRecursivePosition;
  ...
  for (size_t pos = 0; pos < recSOG->numberOfVertices + 1; pos++) {
    ...
    for (size_t mID = 0; mID < methods.size(); mID++) {
      if (spaceNeededInBlocks[mID][pos].second == 0)
        continue; // this does not need space, so ignore it
  ...
  for (size_t mID = 0; mID < methods.size(); mID++) {
    ...
    for (size_t x = 0; x < htn->numSubTasks[m]; x++) {
      ...
      if (!htn->sccIsAcyclic[htn->taskToSCC[task]]) {
        ...
        sog->labels[posInSOG].insert(
            htn->subTasks[m][htn->methodTotalOrder[m][x]]);
        numRec++;
      }
    }
  }
  ...
  for (size_t mID = 0; mID < methods.size(); mID++) {
    ...
    for (size_t x = 0; x < htn->numSubTasks[m] + 1; x++) {
      ...
      if (last || !htn->sccIsAcyclic[htn->taskToSCC[task]]) {
        // cout << "HANDLE " << numRec << " @ " << x << " of " <<
        // htn->numSubTasks[m] << endl;
        int positionOfThisTaskInSOG =
            (!last) ? recSOG->methodSubTasksToVertices[mID][numRec]
                    : (recursivePositions.size() - 1);
        // determine the range
        int startRange = 0;
        // cout << "\t" << lastRecursivePosition << " " <<
        // recursivePositions[lastRecursivePosition] << " " <<
        // recursivePositions.size() << endl;
        ...
        addSequenceToSOG(sog, taskSequence, indexSequence, mID, startRange,
                         endRange);
  ...
  if (false || numRec != methodMaxRec) {
    cout << "SOG: " << numRec << " " << methodMaxRec << " " << methods.size()
         << endl;
      ...
      for (size_t x = 0; x < htn->numSubTasks[m]; x++)
        if (!htn->sccIsAcyclic
                 [htn->taskToSCC[htn->subTasks[m]
                                              [htn->methodTotalOrder[m][x]]]])
          cout << "\t"
               << htn->taskNames[htn->subTasks[m][htn->methodTotalOrder[m][x]]]
               << endl;
    }
  }
...
SOG *optimiseSOG(vector<tuple<uint32_t, uint32_t, uint32_t>> &methods,
                 Model *htn, bool effectLessActionsInSeparateLeaf) {
  // edge case
  ...
  sort(methods.begin(), methods.end(),
       [&](tuple<uint32_t, uint32_t, uint32_t> m1,
           tuple<uint32_t, uint32_t, uint32_t> m2) {
         return htn->numSubTasks[get<0>(m1)] > htn->numSubTasks[get<0>(m2)];
       });

  int maxSize = 0;
  for (size_t mID = 0; mID < methods.size(); mID++)
    if (maxSize < htn->numSubTasks[get<0>(methods[mID])])
      maxSize = htn->numSubTasks[get<0>(methods[mID])];

#ifndef NDEBUG
  if (allMethodsAreTotallyOrdered)
    cout << "Running TO SOG optimiser";
  else
    cout << "Running PO SOG optimiser";
  cout << " with " << methods.size() << " methods with up to " << maxSize
       << " subtasks." << endl;
#endif
  if (allMethodsAreTotallyOrdered)
...
void SOG::removeTransitiveOrderings() {
  ...
  for (int k = 0; k < numberOfVertices; k++)
    for (int x = 0; x < numberOfVertices; x++)
      for (int y = 0; y < numberOfVertices; y++)
        if (adj[x].count(k) && adj[k].count(y))
          trans[x][y] = false;
...
void SOG::precdfs(int n, vector<bool> &visi) {
  ...
  for (int nei : bdj[n]) {
    precdfs(nei, visi);
    predecessorSet[n].insert(predecessorSet[nei].begin(),
                             predecessorSet[nei].end());
  }
}
