...
pair<int, int> printSolution(void *solver, Model *htn, PDT *pdt,
  ...
  //			cout << "Variable " << v << " " << htn->varNames[v] << " is not
  //a SAS+ group ..." << endl; 			cout << "True are:"; 			for (int f : tru) 				cout << "
  //" << f << " " << htn->factStrs[f]; 			cout << endl;
  //
  //			exit(0);
  //		}
  //	}
  ...
  if (htn->isTotallyOrdered) {
    for (PDT *&leaf : leafs) {
      for (size_t pIndex = 0; pIndex < leaf->possiblePrimitives.size();
           pIndex++) {
        ...
        if (ipasir_val(solver, prim) > 0) {
          ...
          std::cout << leaf->outputID << " "
                    << htn->taskNames[leaf->possiblePrimitives[pIndex]] << endl;
          solutionCost += htn->actionCosts[leaf->possiblePrimitives[pIndex]];
          trueSolutionCost +=
              originalActionCosts[leaf->possiblePrimitives[pIndex]];
#ifndef NDEBUG
        ...
        if (ipasir_val(solver, pvar) > 0) {
          ...
          for (int l = 0; l < matching.matchingPerPosition[p].size(); l++) {
            if (ipasir_val(solver, matching.matchingPerPosition[p][l]) > 0 &&
              ...
              std::cout << p << "@" << l << " | " << leaf->outputID << " "
                        << htn->taskNames[prim] << endl;
              cout << "PUP" << endl;
            }
          }
        }
  ...
  cout << "<==" << endl;
  cout << "Total cost of solution: " << solutionCost
       << " true cost: " << trueSolutionCost << endl;
...
bool filter_leafs_ff(vector<PDT *> &leafs, Model *htn) {
  ...
  for (unsigned int l = 0; l < leafs.size(); l++) {
    ...
    for (unsigned int primI = 0; primI < leaf->possiblePrimitives.size();
         primI++) {
      if (leaf->prunedPrimitives[primI])
        continue; // is already pruned
      ...
      if (isExecutable) {
        ...
        // cout << "    Executable at " << l << " prim: " << prim << " " <<
        // htn->taskNames[prim] << endl;
      } else {
        leaf->prunedPrimitives[primI] = true;
        prunedPrimitives++;
        // cout << "Not executable at " << l << " prim: " << prim << " " <<
        // htn->taskNames[prim] << endl;
      }
    }
  ...
  cout << "FF Pruning: removed " << prunedPrimitives << " of "
       << (prunedPrimitives + executablePrimitives) << endl;
  return prunedPrimitives != 0;
}

void insert_invariant(Model *htn, unordered_set<int> *invariants, int a,
                      int b) {
  if (binary_invariants[a + htn->numStateBits].count(b))
    return; // is a global invariant
...
bool filter_leafs_Rintanen(vector<PDT *> &leafs, Model *htn,
                           unordered_set<int> *&after_leaf_invariants,
                           int &additionalInvariants) {
  ...
  compute_Rintanen_initial_invariants(htn, v0, toDelete, posInvarsPerPredicate,
                                      negInvarsPerPredicate);
  ...
  for (unsigned int l = 0; l < leafs.size(); l++) {
    ...
    for (unsigned int primI = 0; primI < leaf->possiblePrimitives.size();
      ...
      bool isExecutable = compute_Rintanten_action_applicable(
          htn, prim, v0, toDelete, posInvarsPerPredicate, negInvarsPerPredicate,
          posInferredPreconditions, negInferredPreconditions);

      if (isExecutable) {
        executable.push_back(prim);
        inferredPreconditions.push_back(
            make_pair(posInferredPreconditions, negInferredPreconditions));
        executablePrimitives++;
        // cout << "    Executable at " << l << " prim: " << prim << " " <<
        // htn->taskNames[prim] << endl;
      } else {
    ...
    for (size_t i = 0; i < executable.size(); i++) {
      ...
      compute_Rintanten_action_effect(
          htn, prim, v0, toDelete, posInvarsPerPredicate, negInvarsPerPredicate,
          posInferredPreconditions, negInferredPreconditions);
    }
    // reduce data structures
    compute_Rintanen_reduce_invariants(htn, v0, toDelete, posInvarsPerPredicate,
                                       negInvarsPerPredicate);
  }
  ...
  cout << "Rintanen Pruning: removed " << prunedPrimitives << " of "
       << (prunedPrimitives + executablePrimitives) << endl;
  return prunedPrimitives != 0;
}

bool createFormulaForDepth(void *solver, PDT *pdt, Model *htn,
                           sat_capsule &capsule, MatchingData &matching,
                           int depth, bool block_compression, bool sat_mutexes,
  ...
  cout << "Computing PDT took: " << setprecision(3) << pdt_time << " ms"
       << endl;
  // printMemory();
  // get leafs
  ...
  */
  ...
  unordered_set<int> *after_leaf_invariants =
      new unordered_set<int>[2 * htn->numStateBits];
  ...
  if (htn->isTotallyOrdered) {
    ...
    for (PDT *leaf : leafs)
      leaf->propagatePruning(htn);

    int pruningPhase = 1;
    int round = 1;
    int additionalInvariants = 0;
    while (pruningMode) {
      cout << color(Color::BLUE, "Pruning round ") << round++
           << " Phase: " << pruningPhase << endl;
      if (pruningPhase == 1) {
        ...
        if (!filter_leafs_Rintanen(leafs, htn, after_leaf_invariants,
    ...
    cout << "Pruning: " << prunedAssignments << " of " << overallAssignments
         << endl;
    overallAssignments = 0;
    prunedAssignments = 0;
    for (PDT *leaf : leafs)
      leaf->countPruning(overallAssignments, prunedAssignments, true);
    cout << "Leaf Primitive Pruning: " << prunedAssignments << " of "
         << overallAssignments << endl;

    // if we have pruned the initial abstract task, return ...
    if (pdt->prunedAbstracts[0])
      return false;

    cout << "Pruning gave " << additionalInvariants << " new invariants"
         << endl;
    // printMemory();
  }
  ...
  cout << " done. " << (capsule.number_of_variables - numVarsBefore)
       << " new variables." << endl;
  // printMemory();
  ...
  // for (PDT* leaf : leafs) leaf->addPrunedClauses(solver); // add assertNo for
  // pruned things
  ...
  if (htn->isTotallyOrdered && block_compression) {
    ...
    cout << "Block compression leads to " << blocks.size() << " timesteps."
         << endl;
#ifndef NDEBUG
  ...
  if (htn->isTotallyOrdered && block_compression)
    generate_state_transition_formula(solver, capsule, vars,
                                      block_base_variables, blocks, htn);
  else
    generate_state_transition_formula(solver, capsule, vars,
                                      block_base_variables, htn);
  ...
  if (sat_mutexes) {
    if (htn->isTotallyOrdered && block_compression)
      generate_mutex_formula(solver, capsule, block_base_variables, blocks,
                             after_leaf_invariants, htn);
    else
      generate_mutex_formula(solver, capsule, block_base_variables,
                             after_leaf_invariants, htn);
  }

  int afterMutex = get_number_of_clauses();

  cout << color(Color::BLUE, "Formula: ") << (afterDecomp - beforeDecomp)
       << " decomposition " << (afterState - afterDecomp) << " state "
       << (afterMutex - afterState) << " mutex" << endl;
  ...
  if (costMode) {
    ...
    for (int t = 0; t < vars.size(); t++) {
      ...
      for (int c = 1; c <= maxCost; c++) {
        ...
        DEBUG(capsule.registerVariable(cvar, "cost var " + pad_int(c) + " @ " +
                                                 pad_int(t)));
        for (const auto &[varID, taskID] : vars[t])
    ...
    cout << "Generating at most K formula " << currentMaximumCost / costFactor
         << "*" << costVars.size() << "="
         << currentMaximumCost / costFactor * costVars.size() << "... " << endl;
...
void bdfs(Model *htn, PDT *cur, PDT *source,
  ...
  if (cur->expanded) {
    ...
    vector<unordered_set<pair<int, int>>> childrenPossibleAssignments(
        cur->children.size());
    ...
    for (auto [tIndex, mIndex] : possibleAssignments) {
      ...
      if (tIndex != -1) {
        ...
        for (size_t child = 0; child < cur->children.size(); child++) {
          if (!cur->getListIndexOfChildrenForMethods(tIndex, mIndex, child)
                   ->present)
            continue;
          bool isPrimitive =
              cur->getListIndexOfChildrenForMethods(tIndex, mIndex, child)
                  ->isPrimitive;
          int subIndex =
              cur->getListIndexOfChildrenForMethods(tIndex, mIndex, child)
                  ->taskIndex;
        ...
        auto [child, tIndex, _] =
            cur->positionOfPrimitivesInChildren[mIndex]; // actually mIndex is
                                                         // the task
    ...
    for (size_t child = 0; child < cur->children.size(); child++) {
      if (cur->children[child] == source)
        continue; // this is the task we come from
...
void temp(Model *htn, PDT *pdt) {
  ...
  for (PDT *l : leafs) {
    for (size_t pI = 0; pI < l->possiblePrimitives.size(); pI++) {
      ...
      cout << "  Computed implications for " << overallAssignments.size()
           << " other vertices." << endl;
      cout << "  extracting mutexes" << endl;
      for (auto &[node, possible] : overallAssignments) {
        for (size_t pIndex = 0; pIndex < node->possiblePrimitives.size();
             pIndex++) {
          // check
          bool canBeAssigned = false;
          for (auto [tIndex, mIndex] : possible)
            canBeAssigned |= (tIndex == -1) && (mIndex == pIndex);
          if (canBeAssigned) {
...
void optimise_with_sat_planner_linear_bound_increase(
    Model *htn, bool block_compression, bool sat_mutexes,
    sat_pruning pruningMode, bool effectLessActionsInSeparateLeaf,
    sat_capsule &capsule, PDT *pdt, int depth, int currentCost) {
  ...
  while (true) {
    ...
    cout << endl
         << endl
         << color(Color::YELLOW, "Generating formula for depth " +
                                     to_string(depth) + " and cost " +
                                     to_string(currentMaximumCost))
         << endl;
    ...
    if (createFormulaForDepth(solver, pdt, htn, capsule, matching, depth,
                              block_compression, sat_mutexes, pruningMode,
                              effectLessActionsInSeparateLeaf)) {
      std::clock_t formula_end = std::clock();
      double formula_time_in_ms =
          1000.0 * (formula_end - formula_start) / CLOCKS_PER_SEC;
      cout << "Formula has " << capsule.number_of_variables << " vars and "
           << get_number_of_clauses() << " clauses." << endl;
      ...
      double solver_time_in_ms =
          1000.0 * (solver_end - solver_start) / CLOCKS_PER_SEC;
      cout << "Solver time: " << fixed << solver_time_in_ms << "ms" << endl;

      if (state != 10 && state != 20) {
        cout << "SAT Solver returned the state " << state
             << " for which we don't know what to do. Exiting." << endl;
        exit(2);
      }

      cout << "Solver state: "
           << color((state == 10 ? Color::GREEN : Color::RED),
                    (state == 10 ? "SAT" : "UNSAT"))
           << endl;
      // if (depth == 3) exit(0);
    } else {
      cout << "Initial abstract task is pruned: " << color(Color::RED, "UNSAT")
           << endl;
    }
...
void solve_with_sat_planner_linear_bound_increase(
    Model *htn, bool block_compression, bool sat_mutexes,
    sat_pruning pruningMode, bool effectLessActionsInSeparateLeaf) {
  PDT *pdt = new PDT(htn);
  ...
  while (true) {
    ...
    cout << endl
         << endl
         << color(Color::YELLOW,
                  "Generating formula for depth " + to_string(depth))
         << endl;
    std::clock_t formula_start = std::clock();
    int state = 20;
    ...
    if (state == 10) {
      ...
      ipasir_release(solver);
      if (optimisePlan)
        optimise_with_sat_planner_linear_bound_increase(
            htn, block_compression, sat_mutexes, pruningMode,
            effectLessActionsInSeparateLeaf, capsule, pdt, depth, apparentCost);
      return;
    } else {
      depth++;
      // return;
...
void *run_sat_planner_for_depth(void *param) {
  ...
  cout << THREAD_PREFIX << "Starting Thread for depth " << ret->depth
       << " @ signal " << ret->signal << endl;
  ...
  cout << THREAD_PREFIX << "I am handled by " << signalBase + ret->signal
       << endl;
  ...
  MatchingData matching;
  createFormulaForDepth(ret->solver, ret->pdt, ret->htn, capsule, matching,
                        ret->depth, ret->block_compression, ret->sat_mutexes,
                        ret->pruningMode, ret->effectLessActionsInSeparateLeaf);
  cout << "Formula has " << capsule.number_of_variables << " vars and "
       << get_number_of_clauses() << " clauses." << endl;
  ...
  if (ret->state != 20) {
    cout << "SAT Solver returned the state " << ret->state
         << " for which we don't know what to do. Exiting." << endl;
    exit(2);
  }
  // nothing to return;
...
void sleep_until_solver_finished(
...
void solve_with_sat_planner_time_interleave(
    Model *htn, bool block_compression, bool sat_mutexes,
    sat_pruning pruningMode, bool effectLessActionsInSeparateLeaf) {
  int maxRuns = 6;
  current_done = false;
  ...
  while (true) {
    ...
    if (positionOnRuns != -1) {
      signal_to_release = -1;
      cout << THREAD_PREFIX << "Stopping " << runs[positionOnRuns]->tid << " @ "
           << runningSingal << endl;
    ...
    cout << THREAD_PREFIX << "Next non-finished task is " << positionOnRuns
         << endl;
    ...
    if (runs.size() == positionOnRuns) { // last run, start a new one
      ...
      int activeRuns = 0;
      for (int i = 0; i < maxRuns; i++)
        takenSignals[i] = false;
      for (thread_returns *t : runs)
        if (!t->done) {
          activeRuns++;
      ...
      if (activeRuns < maxRuns) {
        ...
        pthread_create(&ret->tid, &attr_obj, run_sat_planner_for_depth,
                       (void *)ret);
        cout << THREAD_PREFIX << "Starting worker: " << ret->tid << " @ "
             << firstFreeSignal << endl;
        ...
        // ret->tid); pthread_kill(ret->tid, sig+firstFreeSignal);
        ...
        cout << THREAD_PREFIX
             << "Not possible to start a new run, next non-finished task is "
             << positionOnRuns << endl;
      }
    }

    runningSingal = runs[positionOnRuns]->signal;
    cout << THREAD_PREFIX << "Letting " << positionOnRuns << " work @ "
         << runningSingal << endl;
...
void solve_with_sat_planner(Model *htn, bool block_compression,
  ...
  cout << "Instance is totally ordered: "
       << (htn->isTotallyOrdered ? "yes" : "no") << endl;
  ...
  for (int i = 0; i < htn->numActions; i++)
    originalActionCosts[i] = htn->actionCosts[i];
  ...
  // solve_with_sat_planner_time_interleave(htn, block_compression, sat_mutexes,
  // pruningMode);
  solve_with_sat_planner_linear_bound_increase(htn, block_compression,
                                               sat_mutexes, pruningMode,
                                               effectLessActionsInSeparateLeaf);
}
