...
// XXX magic number, restricts number of copies of a task per task net to 255,
// thereafter the hash becomes incorrect 3 -> up to three bits for (number-1),
// max is 8, thus whole number is max 225
...
struct VisitedList {
  VisitedList(Model *m, bool _noVisitedCheck, bool _noReOpening, bool _taskHash,
              bool _taskSequenceHash, bool _topologicalOrdering,
              bool _orderPairs, bool _layers, bool _allowGIcheck,
              bool _allowedToUseParallelSequences);

  // insert the node into the visited list
  // @returns true if the node was *new* and false, if the node was already
