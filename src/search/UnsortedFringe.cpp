...
void UnsortedFringe::addFirst(searchNode *n) {
  ...
  if (firstContainer->leftIndex == -1) {
    ...
    temp->leftIndex =
        temp->containerSize -
        1; // this is the index that will be used without decrementation
    temp->rightIndex = temp->containerSize; // this will be decremented by the
                                            // remove function before usage
...
void UnsortedFringe::addLast(searchNode *n) {
  ...
  if (lastContainer->rightIndex == lastContainer->containerSize) {
    ...
    temp->leftIndex =
        -1; // this will be incremented by the remove function before usage
    temp->rightIndex =
        0; // this is the index that will be used without incrementation
    this->lastContainer = temp;
  }
}

bool UnsortedFringe::empty() {
  assert((sizeCounter > 0) ||
         ((firstContainer->containerID == lastContainer->containerID) &&
          (lastContainer->leftIndex == lastContainer->rightIndex - 1)));
...
searchNode *UnsortedFringe::removeFirst() {
  ...
  firstContainer->leftIndex++;
  if (firstContainer->leftIndex ==
      firstContainer
          ->containerSize) { // container empty, but there is another one
...
searchNode *UnsortedFringe::removeLast() {
  ...
  lastContainer->rightIndex--;
  if (lastContainer->rightIndex ==
      -1) { // container empty, but there is another one
...
int UnsortedFringe::size() { return sizeCounter; }
