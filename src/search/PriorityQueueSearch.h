...
public:
  ...
  void search(Model *htn, searchNode *tnI, int timeLimit, bool suboptimalSearch,
              bool printSolution, Heuristic **hF, int hLength,
              VisitedList &visitedList, Fringe &fringe) {
    timeval tp;
    ...
    while (!fringe.isEmpty()) {
      ...
      if (!suboptimalSearch && htn->isGoal(n)) {
        // A non-early goal test makes only sense in an optimal planning
        // setting.
      ...
      int allnodes = numSearchNodes + htn->numOneModActions +
                     htn->numOneModMethods + htn->numEffLessProg;
      ...
      if (allnodes - lastCheck >= checkAfter) {
        ...
        if (((currentT - lastOutput) / 1000) > 0) {
          cout << setw(4) << int((currentT - startT) / 1000) << "s "
               << "visitime " << setw(7) << fixed << setprecision(2)
               << visitedList.time / 1000 << "s"
               << " generated nodes " << setw(9) << allnodes << " nodes/sec "
               << setw(7) << int(double(allnodes) / (currentT - startT) * 1000)
               << " cur h " << setw(4) << n->heuristicValue[0] << " mod.depth "
               << setw(4) << n->modificationDepth << " inserts " << setw(9)
               << visitedList.attemptedInsertions << " dups " << setw(9)
               ...
               << " hash buckets " << setw(6)
               << visitedList.attemptedInsertions - visitedList.subHashCollision
               << endl;
          lastOutput = currentT;
        }
    ...
    cout << "- Visited list inserts " << visitedList.attemptedInsertions
         << endl;
    cout << "- Visited list pruned "
         << visitedList.attemptedInsertions - visitedList.uniqueInsertions
         << endl;
    cout << "- Visited list contains " << visitedList.uniqueInsertions << endl;
    cout << "- Visited list hash collisions " << visitedList.subHashCollision
         << endl;
    cout << "- Visited list used hash buckets "
         << visitedList.attemptedInsertions - visitedList.subHashCollision
         << endl;
    cout << "- Generated "
    ...
    cout << "  One modifications "
         << (htn->numOneModActions + htn->numOneModMethods) << endl;
    ...
    if (this->foundSols > 1) {
      ...
      cout << "  - first solution after " << this->firstSolTime << "ms."
           << endl;
...
private:
  searchNode *handleNewSolution(searchNode *newSol,
                                searchNode *globalSolPointer, long time);
