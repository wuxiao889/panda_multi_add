...
const uint64_t tenMillionP =
    16777213;                         // largest prime smaller than 16*1024*1014
...
VisitedList::VisitedList(Model *m, bool _noVisitedCheck, bool _noReOpening,
                         bool _taskHash, bool _taskSequenceHash,
                         bool _topologicalOrdering, bool _orderPairs,
                         bool _layers, bool _allowGIcheck,
                         bool _allowedToUseParallelSequences) {
  this->htn = m;
  ...
  this->useSequencesMode =
      _allowedToUseParallelSequences && this->htn->isParallelSequences;
  this->canDeleteProcessedNodes =
      this->noVisitedCheck || this->useTotalOrderMode || this->useSequencesMode;
  ...
  if (!useSequencesMode)
    ...
    this->bitsNeededPerTask =
        sizeof(int) * 8 -
        __builtin_clz(m->numTasks); // one more ID is needed to separate
                                    // parallel sequences
  ...
  if (this->noVisitedCheck)
    ...
    if (this->topologicalOrdering)
      cout << " topological ordering";
    if (this->orderPairs &&
        !(this->useTotalOrderMode || this->useSequencesMode))
      cout << " order-pairs";
    if (this->layers && !(this->useTotalOrderMode || this->useSequencesMode))
      cout << " layer";
    cout << endl;

    if ((this->useTotalOrderMode || this->useSequencesMode) &&
        !this->topologicalOrdering)
      cout << "- ATTENTION: pruning is " << color(RED, "INCOMPLETE") << endl;
  }
  cout << "- Visited list allows deletion of search nodes: "
       << ((this->canDeleteProcessedNodes) ? "true" : "false") << endl;
}

void dfsdfs(planStep *s, int depth, set<planStep *> &psp,
            set<pair<int, int>> &orderpairs, vector<set<planStep *>> &layer) {
  if (psp.count(s))
    return;
  psp.insert(s);
  if (layer.size() <= depth)
    layer.resize(depth + 1);
...
void to_dfs(planStep *s, vector<int> &seq) {
  ...
  if (s->numSuccessors == 0)
    return;
  to_dfs(s->successorList[0], seq);
}

bool matchingDFS(searchNode *one, searchNode *other, planStep *oneStep,
                 planStep *otherStep, map<planStep *, planStep *> mapping,
                 map<planStep *, planStep *> backmapping);

void getSortedTasks(planStep **network, int size, int *succ);

bool matchingGenerate(searchNode *one, searchNode *other,
  ...
  if (oneNextTasks[*tasks.begin()].size() == 0) {
    ...
    return matchingGenerate(one, other, mapping, backmapping, oneNextTasks,
                            otherNextTasks, tasks);
  }
  ...
  for (planStep *psOther : otherNextTasks[task]) {
    ...
    if (mapping.count(psOne) && mapping[psOne] != psOther)
      continue;
    if (backmapping.count(psOther) && backmapping[psOther] != psOne)
      continue;
    DEBUG(cout << "\tOK" << endl);
    ...
    if (!mapping.count(psOne)) { // don't check again if we already did it
      ...
      subRes =
          matchingDFS(one, other, psOne, psOther, subMapping, subBackmapping);
    }
    ...
    if (subRes) {
      ...
      if (matchingGenerate(one, other, subMapping, subBackmapping, oneNextTasks,
                           subOtherNextTasks, tasks))
        return true;
    }
  }
...
bool matchingDFS(searchNode *one, searchNode *other, planStep *oneStep,
                 planStep *otherStep, map<planStep *, planStep *> mapping,
                 map<planStep *, planStep *> backmapping) {
  if (oneStep->numSuccessors != otherStep->numSuccessors)
    return false; // is not possible
  if (oneStep->numSuccessors == 0)
    return true; // no successors, matching OK
  ...
  for (int t : tasks)
    if (oneNextTasks[t].size() != otherNextTasks[t].size())
      return false;
...
planStep *searchNodePSHead(searchNode *n) {
  ...
  for (int a = 0; a < n->numAbstract; a++)
    ps->successorList[pos++] = n->unconstraintAbstract[a];
  for (int a = 0; a < n->numPrimitive; a++)
    ps->successorList[pos++] = n->unconstraintPrimitive[a];
...
bool matching(searchNode *one, searchNode *other) {
  ...
  bool result =
      matchingDFS(one, other, oneHead, otherHead, mapping, backmapping);
...
// completely ignores order of tasks, but still considers how often specific
// types of tasks occur
...
uint64_t VisitedList::taskSequenceHash(vector<int> &tasks) {
  uint64_t lhash = 0;
  for (int t : tasks) {
    // we need to use +1 here as the task sequence may contain the task
    // htn->numTasks as a divider (for the parallel-seq case)
...
bool VisitedList::insertVisi(searchNode *n) {
  ...
  if (topologicalOrdering || sequenceHash) {
    if (useTotalOrderMode) {
      if (n->numPrimitive)
        to_dfs(n->unconstraintPrimitive[0], sequenceForHashing);
      if (n->numAbstract)
        to_dfs(n->unconstraintAbstract[0], sequenceForHashing);
    }
    if (useSequencesMode) {
      ...
      for (vector<int> &sub : sequences) {
        if (!first)
          sequenceForHashing.push_back(htn->numTasks);
        for (int &s : sub)
          sequenceForHashing.push_back(s);
        first = false;
      }
    } else {
    ...
    if (layers) {
      ...
      for (map<int, int> &m : layerCounts) {
        ...
        for (auto &[task, count] : m) {
          ...
          int bits = (sizeof(int) * 8 - __builtin_clz(number)) -
                     1; // number will never be 0
      ...
      // XXX if we ever add a hash after the layer hash, we must push *two*
      // htn->numTasks in order to ensure a clean boundary
    }
  }
  ...
  // 2. STEP
  // compute the hashs
  uint64_t hash = hash_state_sequence(
      state2Int(n->state).first); // TODO double computation ...
  if (taskHash)
  ...
  compressed_sequence_trie **stateEntry =
      (compressed_sequence_trie **)stateTable->get(hash);
  void **payload;
  if (!*stateEntry)
  ...
  if (useTotalOrderMode || useSequencesMode || !GIcheck) {
    ...
    if (noReopening) {
      *payload = (void *)1; // know the hash is known
      ...
      int costOfInsertedNode =
          n->fValue + 1; // add 1 to distinguish f=0 from no search node at all.
      ...
      if (returnValue)
        *payload = (void *)
            costOfInsertedNode; // now the hash is known at the given cost
    }
