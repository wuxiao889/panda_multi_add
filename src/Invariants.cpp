...
void extract_invariants_from_parsed_model(Model *htn) {
  ...
  for (size_t invarN = 0; invarN < htn->numInvariants; invarN++) {
    if (htn->invariantsSize[invarN] != 2) {
      unusableInvariant++;
      continue;
    }

    insert_invariant(htn, htn->invariants[invarN][0],
                     htn->invariants[invarN][1]);
  }
  ...
  for (size_t mutexN = 0; mutexN < htn->numStrictMutexes; mutexN++) {
    for (size_t m1 = 0; m1 < htn->strictMutexesSize[mutexN]; m1++)
      for (size_t m2 = m1 + 1; m2 < htn->strictMutexesSize[mutexN]; m2++)
        insert_invariant(htn, -htn->strictMutexes[mutexN][m1] - 1,
                         -htn->strictMutexes[mutexN][m2] - 1);

    if (htn->strictMutexesSize[mutexN] != 2) {
      unusableStrictMutexes++;
      continue;
    }
    // if it is strict, one of the two values must be true
    insert_invariant(htn, htn->strictMutexes[mutexN][0],
                     htn->strictMutexes[mutexN][1]);
  }
  ...
  for (size_t mutexN = 0; mutexN < htn->numMutexes; mutexN++) {
    for (size_t m1 = 0; m1 < htn->mutexesSize[mutexN]; m1++)
      for (size_t m2 = m1 + 1; m2 < htn->mutexesSize[mutexN]; m2++)
        insert_invariant(htn, -htn->mutexes[mutexN][m1] - 1,
                         -htn->mutexes[mutexN][m2] - 1);
  }
  ...
  for (size_t var = 0; var < htn->numVars; var++) {
    ...
    if (htn->lastIndex[var] - htn->firstIndex[var] != 1) {
      unusableSASGroups++;
      continue;
    }
  ...
  while (invariant_to_process.size()) {
    while (invariant_to_process.size()) {
      ...
      round++;
      if (round % 500 == 0) {
        cout << "Resolution: " << invariant_to_process.size() << " remaining. "
             << newInvars << " new invariants found." << endl;
      }
    }

    cout << "Starting inference from SAS groups." << endl;
    // the SAS group is essentially a big OR, if we can resolve every member
    // except for one with the same clause we have a new clause
    for (int iAccess = 0; iAccess < 2 * htn->numStateBits; iAccess++) {
      // check for all mutex groups whether it is mutex with all but one of the
      // elements
      for (size_t var = 0; var < htn->numVars; var++) {
        if (htn->firstIndex[var] == htn->lastIndex[var])
          continue; // no true SAS group
        int pred = iAccess - htn->numStateBits;
        if (pred < 0)
          pred = -pred - 1;
        if (htn->firstIndex[var] <= pred && pred <= htn->lastIndex[var])
          continue; // no inference with my own SAS group

        int invar = 0; // 0 all mutex, 1 more than two non mutex
        for (int i = htn->firstIndex[var]; i <= htn->lastIndex[var]; i++) {
          int iNeg = -i - 1;
          if (binary_invariants[iAccess].count(iNeg))
            continue;
          if (invar == 0)
            invar = iNeg;
          else if (invar < 0) {
        ...
        if (invar == 1)
          continue; // cannot be mutex
        if (invar == 0) {
          cout << "Fact " << htn->factStrs[pred] << " (" << pred
               << ") is statically "
               << (iAccess < htn->numStateBits ? "false" : "true") << "."
               << endl;
          cout << "This was shown using the SAS+ variable "
               << htn->varNames[var] << "." << endl;
          cout << "This should have been detected in preprocessing ... I'm "
                  "giving up."
               << endl;
          // exit(0);
        }
        if (invar < 0 && binary_invariants[iAccess].count(-invar - 1) == 0) {
          // so only invar is the only one that is not mutex with iAccess, thus
          // we have a new invariant
          insert_invariant(htn, -invar - 1, iAccess - htn->numStateBits);
          invariant_to_process.push(
              make_pair(-invar - 1, iAccess - htn->numStateBits));
        }
      }
    }
    cout << "Found " << invariant_to_process.size() << " new invariants."
         << endl;
  }
  ...
  double resolution_time =
      1000.0 * (resolution_end - resolution_start) / CLOCKS_PER_SEC;

  cout << "After resolution we have " << number_of_invariants
       << " invariants, taking " << resolution_time << "ms." << endl;
}

void compute_Rintanen_initial_invariants(
    Model *htn, vector<pair<int, int>> &v0, bool *&toDelete,
    ...
    for (int p2 = p1 + 1; p2 < htn->numStateBits; p2++) {
      for (int b1 = 0; b1 < 2; b1++) {
        for (int b2 = 0; b2 < 2; b2++) {
          if (b1 == s0Vector[p1] && b2 == s0Vector[p2])
            continue;
          int l1 = p1;
          if (b1)
            l1 = -p1 - 1;
...
void compute_Rintanen_reduce_invariants(
    Model *htn, vector<pair<int, int>> &v0, bool *&toDelete,
    vector<vector<int>> &posInvarsPerPredicate,
    vector<vector<int>> &negInvarsPerPredicate) {
...
bool compute_Rintanten_action_applicable(
    Model *htn, int tIndex, vector<pair<int, int>> &v0, bool *&toDelete,
    vector<vector<int>> &posInvarsPerPredicate,
    vector<vector<int>> &negInvarsPerPredicate, bool *&posInferredPreconditions,
  ...
  for (size_t preIndex = 0; !inapplicable && preIndex < htn->numPrecs[tIndex];
       preIndex++) {
    ...
    for (size_t invarListIndex = 0;
         invarListIndex < negInvarsPerPredicate[pre].size(); invarListIndex++) {
      ...
      if (v0[invar].first < 0 && pre == -v0[invar].first - 1) {
        if (v0[invar].second < 0) {
          ...
          if (posInferredPreconditions[-v0[invar].second - 1]) {
            inapplicable = true;
            break;
          }
        } else {
          posInferredPreconditions[v0[invar].second] = true;
          if (negInferredPreconditions[v0[invar].second]) {
            inapplicable = true;
            break;
          }
        }
      ...
      if (v0[invar].second < 0 && pre == -v0[invar].second - 1) {
        if (v0[invar].first < 0) {
          ...
          if (posInferredPreconditions[-v0[invar].first - 1]) {
            inapplicable = true;
            break;
          }
        } else {
          posInferredPreconditions[v0[invar].first] = true;
          if (negInferredPreconditions[v0[invar].first]) {
            inapplicable = true;
            break;
          }
        }
...
bool compute_Rintanten_action_effect(Model *htn, int tIndex,
...
#define ensuresNegP(p)                                                         \
  (negInferredPreconditions[p] && !htn->addVectors[tIndex][p]) ||              \
      htn->delVectors[tIndex][p]
#define ensuresPosP(p)                                                         \
  (posInferredPreconditions[p] && !htn->delVectors[tIndex][p]) ||              \
      htn->addVectors[tIndex][p]
  for (int type = 0; type < 2; type++) {
    int num = type ? htn->numAdds[tIndex] : htn->numDels[tIndex];
    int *list = type ? htn->addLists[tIndex] : htn->delLists[tIndex];
    vector<vector<int>> &invarSource =
        type ? negInvarsPerPredicate : posInvarsPerPredicate;

    for (size_t index = 0; index < num; index++) {
      int c = list[index];
      // if the actions adds something, this may violate an invariant containing
      // the predicate negatively
      for (size_t invarListIndex = 0; invarListIndex < invarSource[c].size();
           invarListIndex++) {
        ...
        int ap = v0[invar].first;
        if (!ab)
          ap = -ap - 1;
        int bp = v0[invar].second;
        if (!bb)
          bp = -bp - 1;
...
void compute_Rintanen_Invariants(Model *htn) {
  ...
  do {
    ...
    if (round != 1)
      compute_Rintanen_reduce_invariants(
          htn, v0, toDelete, posInvarsPerPredicate, negInvarsPerPredicate);
    round++;
    ...
    for (size_t tIndex = 0; tIndex < htn->numActions; tIndex++) {
      ...
      if (!nc && tIndex > lastChangeAtAction)
        break;

      if (!compute_Rintanten_action_applicable(
              htn, tIndex, v0, toDelete, posInvarsPerPredicate,
              negInvarsPerPredicate, posInferredPreconditions,
              negInferredPreconditions))
        continue;

      int removedInvariants = compute_Rintanten_action_effect(
          htn, tIndex, v0, toDelete, posInvarsPerPredicate,
          negInvarsPerPredicate, posInferredPreconditions,
          negInferredPreconditions);
      ...
      if (tIndex % 5000 == 4999)
        cout << "  " << v0.size() - nc << " / " << v0.size()
             << " remaining after " << tIndex + 1 << " of " << htn->numActions
             << endl;
    }
  } while (nc);

  std::clock_t invariant_end = std::clock();
  double invariant_time =
      1000.0 * (invariant_end - invariant_start) / CLOCKS_PER_SEC;

  cout << "Found " << v0.size() << " invariants in " << invariant_time << "ms"
       << endl;
  ...
  for (int i = 0; i < 2 * htn->numStateBits; i++) {
    ...
    for (const int &j : binary_invariants[i]) {
      int b = j;
      if (v0set.count(make_pair(a, b)) || v0set.count(make_pair(b, a)))
        continue;
