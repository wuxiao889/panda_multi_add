...
noDelIntSet *noDelIntSet::setUnion(noDelIntSet *second) {
  noDelIntSet *lessBuckets; // this is the number of buckets (Wertebereich), not
                            // the actual size!
...
noDelIntSet *noDelIntSet::setIntersection(noDelIntSet *second) {
  ...
  for (int i = lessElements->getFirst(); i >= 0; i = lessElements->getNext()) {
...
bool noDelIntSet::isEmpty() { return currentSize == 0; }
...
int noDelIntSet::getSize() { return currentSize; }
...
void noDelIntSet::sort() { iu.sort(this->intContainer, 0, currentSize - 1); }
