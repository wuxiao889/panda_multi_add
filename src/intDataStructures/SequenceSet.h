...
struct sequence_trie {
  ...
  sequence_trie(const vector<uint64_t> &sequence, int paddingBits,
                payloadType *&p);
  void insert(const vector<uint64_t> &sequence, int paddingBits,
              payloadType *&p);
...
private:
  ...
  sequence_trie();
  sequence_trie(const vector<uint64_t> &sequence, int startingBlock,
                int startingBit, int paddingBits, payloadType *&p);
