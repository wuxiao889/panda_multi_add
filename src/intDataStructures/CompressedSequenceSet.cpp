...
void compressed_sequence_trie::print_node(int indent) {
  for (int i = 0; i < indent; i++)
    cout << " ";
  cout << blocks << " blocks; block position " << myFirstBlock
       << "; ignore first " << ignoreBitsFirst << "; ignore last "
       << ignoreBitsLast << "; payload " << payload << endl;
...
compressed_sequence_trie::compressed_sequence_trie(
    const vector<uint64_t> &sequence, int paddingBits, void **&p)
...
compressed_sequence_trie::compressed_sequence_trie(
    const vector<uint64_t> &sequence, int startingBlock, int startingBit,
    int paddingBits, void **&p)
    : compressed_sequence_trie() {
...
void compressed_sequence_trie::split_me_at(int block, int bit) {
  DEBUG(cout << "Starting split of block " << block << " @ bit " << bit
             << endl);

  compressed_sequence_trie *child = new compressed_sequence_trie();
  // child gets all blocks from (inclusive) block to blocks
  child->blocks = blocks - block;
  DEBUG(cout << "\tchild receives " << child->blocks << " blocks " << endl);
  assert(child->blocks);
  child->myFirstBlock = myFirstBlock + block;
  DEBUG(cout << "\tfirst block is " << child->myFirstBlock << endl);
  ...
  if (blocks) {
    ...
    // remove last bits
    newValue[blocks - 1] =
        newValue[blocks - 1] & bitmask_ignore_last(ignoreBitsLast);
...
void compressed_sequence_trie::insert(const vector<uint64_t> &sequence,
                                      int paddingBits, void **&p) {
  // all of the padding bits must actually be zero, else the testing of the
  // difference breaks
  assert((sequence.back() & ~bitmask_ignore_last(paddingBits)) == 0);

  DEBUG(cout << endl
             << endl
             << "Calling test and insert; first block " << myFirstBlock << endl;
        print_node(2));

  // find first differing uint64_t
  int block_with_first_difference = -1;
  for (int i = 0; i < blocks; i++) {
    if (myFirstBlock + i >= sequence.size())
      break;
    uint64_t input_test = sequence[myFirstBlock + i];
    DEBUG(cout << "testing block " << i << " (globally " << myFirstBlock + i
               << ": " << bitset<64>(input_test) << ")" << endl);
    if (i == 0)
    ...
    // if the input sequence is shorter then my value, I can only compare up to
    // this length
  ...
  DEBUG(cout << "first block with difference: " << block_with_first_difference
             << endl);
  ...
  if (block_with_first_difference == -1) {
    ...
    if (myFirstBlock + blocks == sequence.size()) {
      ...
      if (ignoreBitsLast == paddingBits) {
        ...
        // continue with child
        uint64_t next_bit_value =
            sequence.back() & bitmask_bit(64 - ignoreBitsLast);
        if (next_bit_value) {
          if (one)
            one->insert(sequence, paddingBits, p);
          else
            one = new compressed_sequence_trie(sequence, sequence.size() - 1,
                                               64 - ignoreBitsLast, paddingBits,
                                               p);
        } else {
          if (zero)
            zero->insert(sequence, paddingBits, p);
          else
            zero = new compressed_sequence_trie(sequence, sequence.size() - 1,
                                                64 - ignoreBitsLast,
                                                paddingBits, p);
        }
        TEST(check_integrity());
      ...
      if (next_bit_value) {
        ...
        if (one) {
          ...
          one = new compressed_sequence_trie(sequence, nextBlock, next_bit,
                                             paddingBits, p);
        }
      } else {
        ...
        if (zero) {
          ...
          DEBUG(cout << "\tCreate." << endl);
          zero = new compressed_sequence_trie(sequence, nextBlock, next_bit,
                                              paddingBits, p);
        }
      }
      TEST(check_integrity());
  ...
  if (block_with_first_difference == 0)
    input_test = input_test & bitmask_ignore_first(ignoreBitsFirst);
  ...
  DEBUG(cout << "Split Block: " << block_with_first_difference << endl;
        cout << "Split Bit  : " << splitBit << endl;);

  // split me
  split_me_at(block_with_first_difference, splitBit);

  if (sequence[myFirstBlock + block_with_first_difference] &
      bitmask_bit(splitBit)) {
    DEBUG(cout << "New Child One. Block "
               << myFirstBlock + block_with_first_difference << endl);
    one = new compressed_sequence_trie(
        sequence, myFirstBlock + block_with_first_difference, splitBit,
        paddingBits, p);
  } else {
    DEBUG(cout << "New Child Two. Block "
               << myFirstBlock + block_with_first_difference << endl);
    zero = new compressed_sequence_trie(
        sequence, myFirstBlock + block_with_first_difference, splitBit,
        paddingBits, p);
  }
...
void stat(compressed_sequence_trie *t, int &nodes, int &fill) {
  ...
  fill += 64 - a - b;
  if (t->zero)
    stat(t->zero, nodes, fill);
  if (t->one)
    stat(t->one, nodes, fill);
}
...
template <> struct hash<vector<uint64_t>> {
...
template <> struct hash<pair<vector<uint64_t>, int>> {
...
void speed_test() {
  ...
  cout << "Trie took: " << setprecision(3) << fixed << trie_time << " ms"
       << endl;
  printMemory();
...
void test() {
  ...
  for (int j = 0; j < 10000; j++) {
    ...
    *(uint64_t *)pay = j + 1;
    ...
    for (auto [key, value] : data) {
      ...
      t->insert(vec, padding, pay);
      DEBUG(cout << "checking "; for (uint64_t v
                                      : vec) cout
                                 << v << ",";
            cout << " pad: " << padding << " @ " << pay << endl);
  ...
  // vector<uint64_t> state = state2Int(vec); state.push_back(55);
  // state.push_back(100); vector<uint64_t> state2 = state2Int(vec);
  ...
  //
  // int* pay;
  // compressed_sequence_trie * t = new compressed_sequence_trie(state2,55,pay);
  // *pay = 10; cout << endl; t->print_tree(0); cout << endl;
  // t->insert(state3,55,pay); *pay = 20;
  // cout << endl;
  // t->print_tree(0);

  // int* pay;
  // compressed_sequence_trie * t = new compressed_sequence_trie(state,55,pay);
  // *pay = 10; cout << endl; t->print_tree(0); cout << endl;
