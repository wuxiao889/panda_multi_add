...
sequence_trie *alloc(uint16_t &block, uint16_t &inside) {
  if (nextCell == cellsPerBlock - 1) {
    ...
    memory_blocks.push_back(
        (sequence_trie *)malloc(sizeof(sequence_trie) * cellsPerBlock));
    if (memory_blocks.size() == 1)
      nextCell++; // 0 should not be a valid address ...
  }
...
sequence_trie *sequence_trie::getOne() { return getCell(oneBlock, oneInside); }

void sequence_trie::print_node(int indent) {
  for (int i = 0; i < indent; i++)
    cout << " ";
  cout << "block position " << myBlock << "; ignore first "
       << int(ignoreBitsFirst) << "; ignore last " << int(ignoreBitsLast)
...
sequence_trie::sequence_trie(const vector<uint64_t> &sequence, int paddingBits,
                             payloadType *&p)
    : sequence_trie(sequence, 0, 0, paddingBits, p) {}

sequence_trie::sequence_trie(const vector<uint64_t> &sequence,
                             int startingBlock, int startingBit,
                             int paddingBits, payloadType *&p)
    : sequence_trie() {
...
void sequence_trie::insert(const vector<uint64_t> &sequence, int paddingBits,
                           payloadType *&p) {
  ...
  DEBUG(cout << endl
             << endl
             << "Calling test and insert; my block " << myBlock << endl;
        print_node(2));
    ...
    if (lastBlockOfInput) {
      ...
      if (ignoreBitsLast == paddingBits) {
        ...
        if (next_bit_value) {
          if (oneBlock || oneInside)
            ...
            sequence_trie *one = alloc(oneBlock, oneInside);
            new (one) sequence_trie(sequence, myBlock, 64 - ignoreBitsLast,
                                    paddingBits, p);
          }
        } else {
          if (zeroBlock || zeroInside)
            ...
            sequence_trie *zero = alloc(zeroBlock, zeroInside);
            new (zero) sequence_trie(sequence, myBlock, 64 - ignoreBitsLast,
                                     paddingBits, p);
          }
        }
        TEST(check_integrity());
      ...
      if (next_bit_value) {
        ...
        if (oneBlock || oneInside) {
          ...
          sequence_trie *one = alloc(oneBlock, oneInside);
          new (one)
              sequence_trie(sequence, nextBlock, next_bit, paddingBits, p);
        }
      } else {
...
void sequence_trie::check_integrity() {
  ...
  if (oneBlock || oneInside)
    getOne()->check_integrity();
  if (zeroBlock || zeroInside)
    getZero()->check_integrity();
}
...
void stat(sequence_trie *t, int &nodes, int &fill) {
  ...
  nodes++;
  fill += 64 - a - b;
  if (t->zeroBlock || t->zeroInside)
    stat(t->getZero(), nodes, fill);
  if (t->oneBlock || t->oneInside)
    stat(t->getOne(), nodes, fill);
}
