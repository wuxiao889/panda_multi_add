...
bool performOneTranslation(HTNToSASTranslation *translation,
                           TranslationType transtype, int parallel,
                           int *pgbList) {
  // time measurement
  timeval tp;
  long startT;
  ...
  if (transtype == Push) {
    ...
    cout << endl
         << "Generating with Basic PO ADL/Conditional effects encoding" << endl;
    operators = translation->htnToCondSorted(pgbList[0]);
  } else {
    // impossible to reach
    assert(false);
...
void printSASToFile(HTNToSASTranslation *translation, TranslationType transtype,
                    string sasfile, bool realCosts) {
  /*
   * Print to file
   */
  timeval tp;
...
int runFD(string sasfile, string solver, string downwardConf,
          string &planFileName) {
  planFileName = sasfile + string(".plan");
  string command = solver + string(" --internal-plan-file ") + planFileName;

  if (downwardConf == "lazy-cea()")
    command += string(" --evaluator 'hcea=cea()' --search 'lazy_greedy([hcea], "
                      "preferred=[hcea])'");
  else if (downwardConf == "ehc-ff()")
    command += string(
        " --evaluator 'hff=ff()' --search 'iterated([ehc(hff, "
        "preferred=[hff]),lazy_greedy([hff], preferred=[hff])], "
        "continue_on_fail=true, continue_on_solve=false, max_time=1800)'");
  else
  ...
  gettimeofday(&tp, NULL);
  currentT = tp.tv_sec * 1000 + tp.tv_usec / 1000;
  cout << "Solving Time (" << (currentT - startT)
       << " ms) with Error Code: " << error_code << endl
       << endl;
...
void runTranslationPlanner(Model *htn, TranslationType transtype,
  ...
  if (!forceTransType) {
    if (htn->isTotallyOrdered) {
      ...
      if (transtype == TO) {
        cout << "- Configuration tried to apply TO encoding to non TO problem "
                "... don't do this. Reverting to Push"
             << endl;
        transtype = Push;
      }
      if (htn->isParallelSequences) {
        ...
        if (transtype == ParallelSeq) {
          cout << "- Configuration tried to apply PSeq encoding to non PSeq "
                  "problem ... don't do this. Reverting to Push"
               << endl;
          transtype = Push;
        }
      }

      // Note: we don't override the base encodings. If you want to use them
      // feel free.
    }
  }
  ...
  while (true) {
    ...
    if (error_code == 3072 || error_code == 2816 || error_code == 512 ||
        error_code == 1280 || error_code > 10) {
      if (!iterate) {
        cout << "- Configuration states not to iterate. Stopping. In order to "
                "iterate over the bound use --iterate"
             << endl;
        exit(1);
      }
