...
void HTNToSASTranslation::sasPlus() {
  ...
  for (int i = 0; i < htn->numVars; i++) {
    for (int j = 0; j < htn->lastIndex[i] - htn->firstIndex[i] + 1; j++) {
      bitsToSP[htn->firstIndex[i] + j] =
          htn->firstIndex[i] + j + sasPlusOffset[i];
      factStrsSP[htn->firstIndex[i] + j + sasPlusOffset[i]] =
          htn->factStrs[htn->firstIndex[i] + j];
    }
    if (sasPlusBits[i]) {
      factStrsSP[htn->firstIndex[i] + 1 + sasPlusOffset[i]] =
          string("no_") + string(htn->factStrs[htn->firstIndex[i]]);
...
int HTNToSASTranslation::calculatePrecsAndAdds(int *s, int *p, int *a,
                                               string tasks, int **conv) {
  ...
  for (int i = 0; i < nT; i++) {
    for (int j = 0; j < htn->numPrecs[task[i]]; j++) {
      varChange[conv[htn->precLists[task[i]][j]][0]][2 * i] =
          htn->precLists[task[i]][j];
    }
    for (int j = 0; j < htn->numAdds[task[i]]; j++) {
      varChange[conv[htn->addLists[task[i]][j]][0]][2 * i + 1] =
          htn->addLists[task[i]][j];
    }
  }
  s[0] = 0;
  s[1] = 0;
  for (int i = 0; i < htn->numVars; i++) {
    ...
    for (int j = 0; j < nT; j++) {
      if (end == -1) {
        ...
        cerr << endl
             << "variable: " << i << ", value old: " << end
             << ", value should be: " << varChange[i][2 * j] << endl;
  ...
  for (int i = 0; i < htn->numMethods; i++) {
    ...
    if (htn->numOrderings[i] == 0) {
      if (i != htn->taskToMethods[htn->initialTask][0] && warning) {
        cout << "ATTENTION: Instance is not of the parallel sequences type. "
                "Search with the PSeq encoding might be incomplete."
             << endl;
      }
      ...
      for (int k = 0; k < htn->numSubTasks[i]; k++) {
        ...
        if (not notAlone) {
          if (not valueSet) {
            ...
            cout << "ATTENTION: Instance is not of the parallel sequences "
                    "type. Search with the PSeq encoding might be incomplete."
                 << endl;
          }
        }
      }
    ...
    for (int j = 0; j < htn->numSubTasks[i]; j++) {
      subTasksInOrder[i][j] =
          htn->subTasks[i][htn->numSubTasks[i] - 1 - subs[j]];
    }
...
int HTNToSASTranslation::maxProgressionBound() { return 5000000; }
...
void HTNToSASTranslation::calcMinimalProgressionBound(bool to) {
  ...
  while (!h.empty()) {
    ...
    if (n->isMethod) {
      ...
      minImpliedPGBM[n->id] =
          htn->numSubTasks[n->id] - off; // number of tasks added to the stack
  ...
  for (int i = 0 ; i < this->numTasks; i++) {
      cout << this->taskNames[i] << " c:" << minImpliedCosts[i] << " d:" <<
  minImpliedDistance[i] << endl;
  }
  */
}
...
int HTNToSASTranslation::htnToCondSorted(int pgb) {
  ...
  for (int i = 0; i < pgb; i++) {
    firstIndexTrans[firstTaskIndex + i] =
        lastIndexTrans[firstTaskIndex + i - 1] + 1;
    lastIndexTrans[firstTaskIndex + i] =
        firstIndexTrans[firstTaskIndex + i] + htn->numTasks;
  }

  for (int i = 0; i < n; i++) {
    firstIndexTrans[firstConstraintIndex + i] =
        lastIndexTrans[firstConstraintIndex + i - 1] + 1;
    lastIndexTrans[firstConstraintIndex + i] =
        firstIndexTrans[firstConstraintIndex + i] + 1;
  }

  for (int i = 0; i < pgb; i++) {
    firstIndexTrans[firstStackIndex + i] =
        lastIndexTrans[firstStackIndex + i - 1] + 1;
    lastIndexTrans[firstStackIndex + i] =
        firstIndexTrans[firstStackIndex + i] + 1;
  }
  for (int i = 0; i < pgb; i++) {
    firstIndexTrans[firstMoveIndex + i] =
        lastIndexTrans[firstMoveIndex + i - 1] + 1;
    lastIndexTrans[firstMoveIndex + i] =
        firstIndexTrans[firstMoveIndex + i] + 1;
  }
  ...
  for (int i = firstTaskIndex; i < firstConstraintIndex; i++) {
    factStrsTrans[firstIndexTrans[i]] = string("+task[point") +
                                        to_string(i - firstTaskIndex) +
                                        string(",noTask]");
    for (int j = 0; j < lastIndexTrans[i] - firstIndexTrans[i]; j++) {
      factStrsTrans[firstIndexTrans[i] + j + 1] =
          string("+task[point") + to_string(i - firstTaskIndex) +
          string(",task") + to_string(j) + ']';
    }
  }
  ...
  for (int i = 0; i < n; i++) {
    factStrsTrans[firstIndexTrans[index]] =
        string("+no_Constraint[") + to_string(k) + ',' + to_string(j) + ']';
    factStrsTrans[firstIndexTrans[index] + 1] =
        string("+Constraint[") + to_string(k) + ',' + to_string(j) + ']';
    j++;
    index++;
    if (j >= k) {
      k++;
  ...
  for (int i = 0; i < pgb; i++) {
    factStrsTrans[firstIndexTrans[i + firstStackIndex]] =
        string("+free[head") + to_string(i) + ']';
    factStrsTrans[firstIndexTrans[i + firstStackIndex] + 1] =
        string("+occupied[head") + to_string(i) + ']';
  }

  for (int i = 0; i < pgb; i++) {
    factStrsTrans[firstIndexTrans[i + firstMoveIndex]] =
        string("+immovable[head") + to_string(i) + ']';
    factStrsTrans[firstIndexTrans[i + firstMoveIndex] + 1] =
        string("+movable[head") + to_string(i) + ']';
  }
  // Initial state
  ...
  s0ListTrans[firstStackIndex - firstTaskIndex] =
      firstIndexTrans[firstStackIndex] + 1;
  ...
  */
  ...
  numActionsTrans =
      (htn->numActions + htn->numMethods) * pgb + htn->numTasks * (pgb - 1);
  ...
  for (int i = 0; i < htn->numMethods; i++) {
    for (int j = 0; j < pgb; j++) {
      ...
      if (htn->numSubTasks[i] > 1) {
        numConditionalEffectsTrans[index] =
            (htn->numSubTasks[i] - 1) * j + htn->numOrderings[i] / 2;
        effectConditionsTrans[index] =
            new int *[numConditionalEffectsTrans[index]];
        numEffectConditionsTrans[index] =
            new int[numConditionalEffectsTrans[index]];
        ...
        for (int l = 0; l < numConditionalEffectsTrans[index]; l++) {
          ...
          effectConditionsTrans[index][l] =
              new int[numEffectConditionsTrans[index][l]];
        }
      } else {
  ...
  for (int i = 0; i < htn->numActions; i++) {
    for (int j = 0; j < pgb; j++) {
      ...
      for (int k = 0; k < pgb; k++) {
        if (k < j) {
          precListsTrans[index][htn->numPrecs[i] + k] =
              firstIndexTrans[firstStackIndex + k] + 1;
          addListsTrans[index][htn->numAdds[i] + k] =
              firstIndexTrans[firstConstraintIndex + j * (j - 1) / 2 + k];
        } else if (k > j) {
          precListsTrans[index][htn->numPrecs[i] + k] =
              firstIndexTrans[firstConstraintIndex + k * (k - 1) / 2 + j];
        } else {
          precListsTrans[index][htn->numPrecs[i] + k] =
              firstIndexTrans[firstTaskIndex + j] + i + 1;
        }
      }
      addListsTrans[index][htn->numAdds[i] + j] =
          firstIndexTrans[firstTaskIndex + j];
      addListsTrans[index][htn->numAdds[i] + j + 1] =
          firstIndexTrans[firstStackIndex + j];
    }
  }
    ...
    for (int j = 0; j < pgb; j++) {
      ...
      for (int k = 0; k < pgb; k++) {
        if (k < j) {
          ...
          precListsTrans[index][k] =
              firstIndexTrans[firstConstraintIndex + k * (k - 1) / 2 + j];
        } else {
          precListsTrans[index][k] =
              firstIndexTrans[firstTaskIndex + j] + taskToKill[i];
        }
      }

      if (htn->numSubTasks[i] == 0) {
        for (int k = 0; k < j; k++) {
          addListsTrans[index][k] =
              firstIndexTrans[firstConstraintIndex + j * (j - 1) / 2 + k];
        }
        addListsTrans[index][j] = firstIndexTrans[firstTaskIndex + j];
        addListsTrans[index][j + 1] = firstIndexTrans[firstStackIndex + j];
      } else if (htn->numSubTasks[i] == 1) {
        addListsTrans[index][0] =
            firstIndexTrans[firstTaskIndex + j] + 1 + subTasksInOrder[i][0];
      } else {
        addListsTrans[index][0] =
            firstIndexTrans[firstTaskIndex + j] + 1 + subTasksInOrder[i][0];
        for (int l = 0; l < htn->numSubTasks[i] - 1; l++) {
          precListsTrans[index][l + pgb] =
              firstIndexTrans[firstTaskIndex + pgb - 1 - l];
          addListsTrans[index][l + 1] =
              firstIndexTrans[firstTaskIndex + pgb - htn->numSubTasks[i] + 1 +
                              l] +
              1 + subTasksInOrder[i][l + 1];
          addListsTrans[index][l + htn->numSubTasks[i]] =
              firstIndexTrans[firstStackIndex + pgb - 1 - l] + 1;
          for (int m = 0; m < j; m++) {
            effectConditionsTrans[index][l * j + m][0] =
                firstIndexTrans[firstConstraintIndex + j * (j - 1) / 2 + m] + 1;
            effectsTrans[index][l * j + m] =
                firstIndexTrans[firstConstraintIndex +
                                (pgb - l - 1) * (pgb - l - 2) / 2 + m] +
                1;
          }
        }
        for (int l = 0; l < htn->numOrderings[i] / 2; l++) {
          ...
          effectConditionsTrans[index][j * (htn->numSubTasks[i] - 1) + l][0] =
              firstIndexTrans[firstConstraintIndex + first * (first - 1) / 2 +
                              second];
          effectsTrans[index][j * (htn->numSubTasks[i] - 1) + l] =
              firstIndexTrans[firstConstraintIndex + first * (first - 1) / 2 +
                              second] +
              1;
        }
    ...
    for (int j = 1; j < pgb; j++) {
      ...
      for (int l = 0; l < pgb - 2; l++) {
        if (l < j) {
          ...
          precListsTrans[index][4 + l] =
              firstIndexTrans[firstMoveIndex + l + 1];
        }
        if (l < j - 1) {
          addListsTrans[index][5 + l] =
              firstIndexTrans[firstConstraintIndex + j * (j - 1) / 2 + l];
          effectConditionsTrans[index][l][0] =
              firstIndexTrans[firstConstraintIndex + j * (j - 1) / 2 + l] + 1;
          effectsTrans[index][l] = firstIndexTrans[firstConstraintIndex +
                                                   (j - 1) * (j - 2) / 2 + l] +
                                   1;
        } else {
          addListsTrans[index][5 + l] =
              firstIndexTrans[firstConstraintIndex + (l + 2) * (l + 1) / 2 + j];
          effectConditionsTrans[index][l][0] =
              firstIndexTrans[firstConstraintIndex + (l + 2) * (l + 1) / 2 +
                              j] +
              1;
          effectsTrans[index][l] =
              firstIndexTrans[firstConstraintIndex + (l + 2) * (l + 1) / 2 + j -
                              1] +
              1;
        }
      ...
      if (j > 1) {
        effectConditionsTrans[index][pgb - 2][0] =
            firstIndexTrans[firstStackIndex + j - 2];
        effectsTrans[index][pgb - 2] =
            firstIndexTrans[firstMoveIndex + j - 1] + 1;
        effectConditionsTrans[index][pgb - 1][0] =
            firstIndexTrans[firstStackIndex + j - 2] + 1;
  ...
  for (int i = 0; i < htn->numActions; i++) {
    for (int j = 0; j < pgb; j++) {
      actionNamesTrans[i * pgb + j] = "primitive(id[" + to_string(i) +
                                      "],head[" + to_string(j) +
                                      "]): " + htn->taskNames[i];
    }
  }
  for (int i = 0; i < htn->numMethods; i++) {
    for (int j = 0; j < pgb; j++) {
      ...
      actionNamesTrans[index] =
          "method(id[" + to_string(i) + "],head[" + to_string(j);
      if (htn->numSubTasks[i] > 0) {
        ...
        for (int l = 0; l < htn->numSubTasks[i] - 1; l++) {
          ...
          actionNamesTrans[index] +=
              to_string(pgb - htn->numSubTasks[i] + 1 + l);
        }
        actionNamesTrans[index] += "]): ";
  ...
  for (int i = 0; i < htn->numTasks; i++) {
    for (int j = 1; j < pgb; j++) {
      actionNamesTrans[index] = "move(task[" + to_string(i) + "],from[" +
                                to_string(j) + "],to[" + to_string(j - 1) + "]";
      index++;
    }
  }
...
int HTNToSASTranslation::tohtnToStrips(int pgb) {
  ...
  for (int i = 0; i < pgb; i++) {
    ...
    lastIndexTrans[firstTaskIndex + i] =
        firstIndexTrans[firstTaskIndex + i] + htn->numTasks;
    // cout << firstIndexTrans[firstTaskIndex + i] << " " <<
    // lastIndexTrans[firstTaskIndex + i] << endl;
  }
  ...
  for (int i = 0; i < pgb; i++) {
    factStrsTrans[i + firstIndexTrans[headIndex] + 1] =
        string("ppoint(head, point") + to_string(i) + ')';
  }
  for (int i = firstTaskIndex; i < numVarsTrans; i++) {
    factStrsTrans[firstIndexTrans[i]] = string("ptask(point") +
                                        to_string(i - firstTaskIndex) +
                                        string(", noTask)");
    for (int j = 0; j < lastIndexTrans[i] - firstIndexTrans[i]; j++) {
      factStrsTrans[firstIndexTrans[i] + j + 1] =
          string("ptask(point") + to_string(i - firstTaskIndex) +
          string(", task") + to_string(j) + ')';
    }
  }
  ...
  for (int i = firstMethodIndex; i < numActionsTrans; i++) {
    ...
    // cout << "ACC " << i << " " << method << " " <<
    // firstNumTOPrimTasks[method] << " " << numSubTasks[method] << " " <<
    // firstNumTOPrimTasks[method] << endl;
    if (firstNumTOPrimTasks[method] > 0) {
      for (int k = 0; k < firstNumTOPrimTasks[method]; k++) {
        // cout << "C " << method << " " << numMethods << " " <<
        // numSubTasks[method] << " " << 1 << " " << k << " = " <<
        // subTasksInOrder[method][numSubTasks[method] - 1 - k] << endl;
        actionCostsTrans[i] +=
            htn->actionCosts[subTasksInOrder[method]
                                            [htn->numSubTasks[method] - 1 - k]];
      }
    }
  }
  ...
  for (int i = 0; i < htn->numMethods; i++) {
    for (int j = 0; j < pgb; j++) {
      ...
      if (htn->numSubTasks[i] - firstNumTOPrimTasks[i] == 0) {
        ...
        numAddsTrans[firstMethodIndex + i * pgb + j] =
            htn->numSubTasks[i] - firstNumTOPrimTasks[i] + 1;
      }
    }
  }
  ...
  for (int i = 0; i < htn->numActions; i++) {
    for (int j = 0; j < pgb; j++) {
      ...
      precListsTrans[i * pgb + j][numPrecsTrans[i * pgb + j] - 1] =
          firstIndexTrans[headIndex] + 1 + j;
      precListsTrans[i * pgb + j][numPrecsTrans[i * pgb + j] - 2] =
          firstIndexTrans[firstTaskIndex + j] + 1 + i;
      for (int k = 0; k < htn->numAdds[i]; k++) {
        addListsTrans[i * pgb + j][k] = htn->addLists[i][k];
      }
      addListsTrans[i * pgb + j][numAddsTrans[i * pgb + j] - 1] =
          firstIndexTrans[headIndex] + j;
      addListsTrans[i * pgb + j][numAddsTrans[i * pgb + j] - 2] =
          firstIndexTrans[firstTaskIndex + j];
    }
  }
  taskToKill = new int[htn->numMethods];
  ...
  for (int i = 0; i < htn->numMethods; i++) {
    for (int j = 0; j < pgb; j++) {
      ...
      precListsTrans[firstMethodIndex + i * pgb + j][0] =
          firstIndexTrans[headIndex] + 1 + j;
      precListsTrans[firstMethodIndex + i * pgb + j][1] =
          firstIndexTrans[firstTaskIndex + j] + taskToKill[i];
      if (htn->numSubTasks[i] - firstNumTOPrimTasks[i] == 0) {
        addListsTrans[firstMethodIndex + i * pgb + j][1] =
            firstIndexTrans[firstTaskIndex + j];
        addListsTrans[firstMethodIndex + i * pgb + j][0] =
            firstIndexTrans[headIndex] + j;
        continue;
      }
      ...
      for (int k = 0; k < htn->numSubTasks[i] - firstNumTOPrimTasks[i]; k++) {
        addListsTrans[firstMethodIndex + i * pgb + j][k] =
            firstIndexTrans[firstTaskIndex + j + k] + 1 + subTasksInOrder[i][k];
      }
      addListsTrans[firstMethodIndex + i * pgb + j]
                   [numAddsTrans[firstMethodIndex + i * pgb + j] - 1] =
                       firstIndexTrans[headIndex] + j + htn->numSubTasks[i] -
                       firstNumTOPrimTasks[i];
    }
  }
  // names
  ...
  for (int i = 0; i < htn->numMethods; i++) {
    for (int j = 0; j < pgb; j++) {
      actionNamesTrans[firstMethodIndex + i * pgb + j] =
          "method(id[" + to_string(i) + "],head[" + to_string(j);
      if (htn->numSubTasks[i] - firstNumTOPrimTasks[i] > 0) {
        actionNamesTrans[firstMethodIndex + i * pgb + j] +=
            "],subtasks[" + to_string(j);
      ...
      if (firstNumTOPrimTasks[i] > 0) {
        actionNamesTrans[firstMethodIndex + i * pgb + j] +=
            "],firstPrim[" +
            to_string(subTasksInOrder[i][htn->numSubTasks[i] - 1]);
        for (int k = 1; k < firstNumTOPrimTasks[i]; k++) {
          actionNamesTrans[firstMethodIndex + i * pgb + j] +=
              ',' + to_string(subTasksInOrder[i][htn->numSubTasks[i] - 1 - k]);
        }
      }
      actionNamesTrans[firstMethodIndex + i * pgb + j] +=
          "]): " + htn->methodNames[i];
      // cout << "output " << i << " " << j << " " << firstMethodIndex + i * pgb
      // + j << " " << " " << firstNumTOPrimTasks[i] <<
      // actionNamesTrans[firstMethodIndex + i * pgb + j] << endl;
    }
  }
  return numActionsTrans;
}
...
int HTNToSASTranslation::htnPS(int numSeq, int *pgbList) {
  // number of variables: one head counter per sequence, the sequences
  // themselves and constraints between sequences
  int pgb = 0;
  ...
  for (int i = 0; i < numSeq; i++) {
    varNamesTrans[index] =
        "var" + to_string(index) + ",sequence" + to_string(i);
    index++;
  }
  ...
  for (int i = 0; i < numSeq; i++) {
    for (int j = 0; j < pgbList[i]; j++) {
      varNamesTrans[index] = "var" + to_string(index) + ",sequence" +
                             to_string(i) + "head" + to_string(j);
      index++;
    }
  }
  ...
  for (int i = 0; i < numSeq; i++) {
    for (int j = 0; j < numSeq - 1; j++) {
      varNamesTrans[index] = "var" + to_string(index) + ",sequence" +
                             to_string(i) + "constrains" + to_string(j);
      index++;
    }
  }
  for (int i = 0; i < htn->numVars; i++) {
  ...
  for (int i = 0; i < numSeq; i++) {
    for (int j = 0; j < numSeq; j++) {
      ...
      factStrsTrans[index] = string("+unconstrained[sequence") + to_string(i) +
                             string(",sequence") + to_string(j) + ']';
      index++;
      factStrsTrans[index] = string("+constrained[sequence") + to_string(i) +
                             string(",sequence") + to_string(j) + ']';
      index++;
    }
  }
  ...
  for (int i = 0; i < numSeq; i++) {
    for (int j = 0; j < pgbList[i]; j++) {
      factStrsTrans[index] = string("+task[sequence") + to_string(i) +
  ...
  // transformed actions and methods
  numActionsTrans =
      htn->numActions * pgb * numSeq + htn->numMethods * pgb * numSeq;
    ...
    for (int j = 0; j < pgb; j++) {
      for (int k = 0; k < numSeq; k++) {
        ...
        for (int l = 0; l < numSeq - 1; l++) {
          ...
          precListsTrans[index][htn->numPrecs[i] + 2 + l] =
              firstIndexTrans[firstConstraintIndex + (l + off) * (numSeq - 1) +
                              k - 1 + off];
          if (j == 0) {
            addListsTrans[index][htn->numAdds[i] + 2 + l] =
                firstIndexTrans[firstConstraintIndex + (numSeq - 1) * k + l];
          }
        }
        precListsTrans[index][htn->numPrecs[i]] =
            firstIndexTrans[headIndex + k] + 1 + j;
        precListsTrans[index][htn->numPrecs[i] + 1] =
            firstIndexTrans[taskIndezes[k] + j] + 1 + i;
        addListsTrans[index][htn->numAdds[i]] =
            firstIndexTrans[headIndex + k] + j;
        addListsTrans[index][htn->numAdds[i] + 1] =
            firstIndexTrans[taskIndezes[k] + j];

        actionNamesTrans[index] = "primitive(id[" + to_string(i) + "],head[" +
                                  to_string(k * pgb + j) +
                                  "]): " + htn->taskNames[i];
        index++;
      }
  ...
  for (int i = 0; i < htn->numMethods; i++) {
    for (int j = 0; j < pgb; j++) {
      for (int k = 0; k < numSeq; k++) {
        ...
        if (htn->numSubTasks[i] + j > pgbList[k] &&
            !(taskToKill[i] == htn->initialTask + 1)) {
          invalidTransActions[index] = true;
          numInvalidTransActions++;
          index++;
          continue;
        ...
        if (taskToKill[i] == htn->initialTask + 1) {
          if (j == 0 && k == 0) {
            ...
            for (int l = 0; l < numSeq; l++) {
              ...
              addListsTrans[index][1 + l + numSeq] =
                  firstIndexTrans[taskIndezes[l]] + subTasksInOrder[i][l] + 1;
              for (int m = 0; m < numSeq - 1; m++) {
                addListsTrans[index][1 + 2 * numSeq + l * (numSeq - 1) + m] =
                    firstIndexTrans[firstConstraintIndex + l * (numSeq - 1) +
                                    m];
              }
            }
            for (int l = 0; l < htn->numOrderings[i] / 2; l++) {
              ...
              int order = htn->ordering[i][2 * l] * (numSeq - 1) +
                          htn->ordering[i][2 * l + 1] - off;
              addListsTrans[index][1 + 2 * numSeq + order] =
                  firstIndexTrans[firstConstraintIndex + order] + 1;
            }
          } else {
            invalidTransActions[index] = true;
            numInvalidTransActions++;
          ...
          actionNamesTrans[index] =
              "method(id[" + to_string(i) + "],head[" + to_string(k * pgb + j);
        ...
        precListsTrans[index][1] =
            firstIndexTrans[taskIndezes[k] + j] + taskToKill[i];
        for (int l = 0; l < numSeq - 1; l++) {
          int off = 0;
          if (l >= k) {
            off = 1;
          }
          precListsTrans[index][2 + l] =
              firstIndexTrans[firstConstraintIndex + (l + off) * (numSeq - 1) +
                              k - 1 + off];
          if (j == 0 && htn->numSubTasks[i] == 0) {
            addListsTrans[index][2 + l] =
                firstIndexTrans[firstConstraintIndex + (numSeq - 1) * k + l];
          }
        }
        if (htn->numSubTasks[i] == 0) {
          ...
          addListsTrans[index][0] =
              firstIndexTrans[taskIndezes[k] + j] + 1 + subTasksInOrder[i][0];
        } else {
          for (int l = 0; l < htn->numSubTasks[i]; l++) {
            addListsTrans[index][l] = firstIndexTrans[taskIndezes[k] + j + l] +
                                      1 + subTasksInOrder[i][l];
          }
          addListsTrans[index][numAddsTrans[index] - 1] =
              firstIndexTrans[headIndex + k] + j + htn->numSubTasks[i];
        }
...
int HTNToSASTranslation::htnToStrips(int pgb) {
  ...
  for (int i = 0; i < pgb; i++) {
    firstIndexTrans[firstTaskIndex + i] =
        lastIndexTrans[firstTaskIndex + i - 1] + 1;
    lastIndexTrans[firstTaskIndex + i] =
        firstIndexTrans[firstTaskIndex + i] + htn->numTasks + 1;
  }
  ...
  for (int i = firstTaskIndex; i < firstConstraintIndex; i++) {
    factStrsTrans[firstIndexTrans[i]] = string("+task[head") +
                                        to_string(i - firstTaskIndex) +
                                        string(",noTask]");
    factStrsTrans[firstIndexTrans[i] + 1] = string("+task[head") +
                                            to_string(i - firstTaskIndex) +
                                            string(",emptyTask]");
    for (int j = 0; j < lastIndexTrans[i] - firstIndexTrans[i] - 1; j++) {
      factStrsTrans[firstIndexTrans[i] + j + 1] =
          string("+task[head") + to_string(i - firstTaskIndex) +
          string(",task") + to_string(j) + ']';
    }
  }
  int j = 0;
  ...
  for (int i = firstConstraintIndex; i < firstStackIndex; i++) {
    ...
    factStrsTrans[firstIndexTrans[i]] =
        string("+no_Constraint[") + to_string(k) + ',' + to_string(j) + ']';
    factStrsTrans[firstIndexTrans[i] + 1] =
        string("+Constraint[") + to_string(k) + ',' + to_string(j) + ']';
  }
  for (int i = 0; i < pgb; i++) {
    factStrsTrans[firstIndexTrans[i + firstStackIndex]] =
  ...
  for (int i = 0; i < pgb; i++) {
    ...
    for (int k = 0; k < pgb; k++) {
      if (k < i) {
        emptyTaskPrecs[i][k] =
            firstIndexTrans[firstConstraintIndex + k * (pgb - 1) + i - 1];
        emptyTaskAdds[i][k] =
            firstIndexTrans[firstConstraintIndex + i * (pgb - 1) + k];
      } else if (i == k) {
        emptyTaskPrecs[i][k] = firstIndexTrans[firstTaskIndex + i] + 1;
        emptyTaskAdds[i][k] = firstIndexTrans[firstTaskIndex + i];
      } else {
        emptyTaskPrecs[i][k] =
            firstIndexTrans[firstConstraintIndex + k * (pgb - 1) + i];
        emptyTaskAdds[i][k] =
            firstIndexTrans[firstConstraintIndex + i * (pgb - 1) + k - 1];
      }
    }
  ...
  for (int i = 0; i < htn->numActions; i++) {
    for (int j = 0; j < pgb; j++) {
      ...
      for (int l = 0; l < pgb; l++) {
        if (l < j) {
          precListsTrans[index][htn->numPrecs[i] + l] =
              firstIndexTrans[firstConstraintIndex + l * (pgb - 1) + j - 1];
        } else if (j == l) {
          precListsTrans[index][htn->numPrecs[i] + l] =
              firstIndexTrans[firstTaskIndex + j] + 2 + i;
        } else {
          precListsTrans[index][htn->numPrecs[i] + l] =
              firstIndexTrans[firstConstraintIndex + l * (pgb - 1) + j];
        }
      }
      actionNamesTrans[index] = "primitive(id[" + to_string(i) + "],head[" +
                                to_string(j) + "]): " + htn->taskNames[i];
      addListsTrans[index][htn->numAdds[i]] =
          firstIndexTrans[firstTaskIndex + j];
      addListsTrans[index][numAddsTrans[index] - 1] =
          firstIndexTrans[firstStackIndex + j];
      for (int k = 0; k < pgb - 1; k++) {
        addListsTrans[index][htn->numAdds[i] + 1 + k] =
            firstIndexTrans[firstConstraintIndex + j * (pgb - 1) + k];
      }
    }
  }
  ...
  for (int i = 0; i < htn->numMethods; i++) {
    ...
    for (int j = 0; j < pgb; j++) {
      for (int k = 0; k < (methodIndexes[i + 1] - methodIndexes[i]); k++) {
        int index = firstMethodIndex + methodIndexes[i] * pgb +
                    j * (methodIndexes[i + 1] - methodIndexes[i]) + k;
        int inv = 0;
        if (htn->numSubTasks[i] == 1) {
          numAddsTrans[index] = 1;
          numPrecsTrans[index] = pgb;
          ...
          if (hasNoLastTask[i]) {
            ...
            numAddsTrans[index] =
                htn->numSubTasks[i] * 3 + 1 + htn->numOrderings[i] / 2;
            numPrecsTrans[index] =
                htn->numSubTasks[i] + 1 + pgb + subs[htn->numSubTasks[i] - 1];
          } else {
            combination(subs, pgb - 1, htn->numSubTasks[i] - 1, k);
            numAddsTrans[index] =
                htn->numSubTasks[i] * 2 - 1 + htn->numOrderings[i] / 2;
            numPrecsTrans[index] =
                htn->numSubTasks[i] + pgb + subs[htn->numSubTasks[i] - 2];
          }
        }
        precListsTrans[index] = new int[numPrecsTrans[index]];
        addListsTrans[index] = new int[numAddsTrans[index]];
        ...
        for (int m = 0; m < pgb; m++) {
          if (m < j) {
            precListsTrans[index][m] =
                firstIndexTrans[firstConstraintIndex + m * (pgb - 1) + j - 1];
          } else if (j == m) {
            precListsTrans[index][m] =
                firstIndexTrans[firstTaskIndex + m] + taskToKill[i];
          } else {
            precListsTrans[index][m] =
                firstIndexTrans[firstConstraintIndex + m * (pgb - 1) + j];
          }
        }
        if (htn->numSubTasks[i] == 0) {
          addListsTrans[index][0] = firstIndexTrans[firstTaskIndex + j];
          ...
          for (int m = 0; m < pgb - 1; m++) {
            addListsTrans[index][1 + m] =
                firstIndexTrans[firstConstraintIndex + j * (pgb - 1) + m];
          }
        } else if (htn->numSubTasks[i] == 1) {
          addListsTrans[index][0] =
              firstIndexTrans[firstTaskIndex + j] + 2 + subTasksInOrder[i][0];
        } else {
          if (hasNoLastTask[i]) {
            ...
            for (int m = 0; m < subs[htn->numSubTasks[i] - 1] + 1; m++) {
              ...
              precListsTrans[index][htn->numSubTasks[i] + pgb + m] =
                  firstIndexTrans[firstStackIndex + m + off] + 1;
            }
            for (int m = 0; m < htn->numSubTasks[i]; m++) {
              ...
              precListsTrans[index][m + pgb] =
                  firstIndexTrans[firstTaskIndex + subs[m]];
              precListsTrans[index][htn->numSubTasks[i] + pgb + subs[m] - off] =
                  firstIndexTrans[firstStackIndex + subs[m]];
              addListsTrans[index][m + 1] =
                  firstIndexTrans[firstTaskIndex + subs[m]] + 2 +
                  subTasksInOrder[i][m];
              addListsTrans[index][m + htn->numSubTasks[i] + 1] =
                  firstIndexTrans[firstConstraintIndex + (subs[m]) * (pgb - 1) +
                                  j - 1 + off] +
                  1;
              addListsTrans[index][m + htn->numSubTasks[i] * 2 + 1] =
                  firstIndexTrans[firstStackIndex + (subs[m])] + 1;
            }
            for (int m = 0; m < htn->numOrderings[i] / 2; m++) {
              ...
              addListsTrans[index][m + htn->numSubTasks[i] * 3 + 1] =
                  lastIndexTrans[firstConstraintIndex + subs[free] * (pgb - 1) +
                                 subs[constrained] - first];
            }
          } else {
            addListsTrans[index][0] =
                firstIndexTrans[firstTaskIndex + j] + subTasksInOrder[i][0] + 2;
            for (int m = 0; m < subs[htn->numSubTasks[i] - 2] + 1; m++) {
              int off = 0;
              if (m >= j) {
                off = 1;
              }
              precListsTrans[index][htn->numSubTasks[i] + pgb - 1 + m] =
                  firstIndexTrans[firstStackIndex + m + off] + 1;
            }
            for (int m = 0; m < htn->numSubTasks[i] - 1; m++) {
              ...
              precListsTrans[index][m + pgb] =
                  firstIndexTrans[firstTaskIndex + subs[m]];
              precListsTrans[index]
                            [htn->numSubTasks[i] + pgb - 1 + subs[m] - off] =
                                firstIndexTrans[firstStackIndex + subs[m]];
              addListsTrans[index][m + 1] =
                  firstIndexTrans[firstTaskIndex + subs[m]] + 2 +
                  subTasksInOrder[i][m + 1];
              addListsTrans[index][m + htn->numSubTasks[i]] =
                  firstIndexTrans[firstStackIndex + subs[m]] + 1;
            }
            for (int m = 0; m < htn->numOrderings[i] / 2; m++) {
              int first = 0;
              ...
              if (constrained == -1) {
                ...
                addListsTrans[index][m + htn->numSubTasks[i] * 2 - 1] =
                    lastIndexTrans[firstConstraintIndex +
                                   subs[free] * (pgb - 1) + j - first];
              } else {
                if (subs[constrained] >= subs[free]) {
                  first = 1;
                }
                addListsTrans[index][m + htn->numSubTasks[i] * 2 - 1] =
                    lastIndexTrans[firstConstraintIndex +
                                   subs[free] * (pgb - 1) + subs[constrained] -
                                   first];
              }
            }
          }
        }
        string name = "method(id[" + to_string(i) + "],head[" + to_string(j) +
                      "],subtasks[";
        if (hasNoLastTask[i]) {
          for (int l = 0; l < htn->numSubTasks[i]; l++) {
  ...
  for (int i = 0; i < htn->numMethods; i++) {
    for (int j = 0; j < pgb; j++) {
      for (int k = 0; k < (methodIndexes[i + 1] - methodIndexes[i]); k++) {
        ...
        if (htn->numSubTasks[i] > 1) {
          numConditionalEffectsTrans[index] =
              (htn->numSubTasks[i] - 1) * (pgb - 1) + htn->numOrderings[i] / 2;
          effectConditionsTrans[index] =
              new int *[numConditionalEffectsTrans[index]];
          numEffectConditionsTrans[index] =
              new int[numConditionalEffectsTrans[index]];
  ...
  for (int i = 0; i < htn->numActions; i++) {
    for (int j = 0; j < pgb; j++) {
      ...
      for (int k = 0; k < pgb; k++) {
        if (k < j) {
          precListsTrans[index][htn->numPrecs[i] + k] =
              firstIndexTrans[firstConstraintIndex + k * (pgb - 1) + j - 1];
        } else if (j == k) {
          precListsTrans[index][htn->numPrecs[i] + k] =
              firstIndexTrans[firstTaskIndex + j] + 1 + i;
        } else {
          precListsTrans[index][htn->numPrecs[i] + k] =
              firstIndexTrans[firstConstraintIndex + k * (pgb - 1) + j];
        }
      }
      for (int k = 0; k < pgb - 1; k++) {
        addListsTrans[index][htn->numAdds[i] + k] =
      ...
      addListsTrans[index][htn->numAdds[i] + pgb - 1] =
          firstIndexTrans[firstTaskIndex + j];
      addListsTrans[index][htn->numAdds[i] + pgb] =
          firstIndexTrans[firstStackIndex + j];
    }
  }
      ...
      for (int k = 0; k < (methodIndexes[i + 1] - methodIndexes[i]); k++) {
        ...
        if (htn->numSubTasks[i] == 0) {
          ...
          for (int l = 0; l < pgb - 1; l++) {
            addListsTrans[index][l + 1] =
                firstIndexTrans[firstConstraintIndex + j * (pgb - 1) + l];
          }
          addListsTrans[index][pgb] = firstIndexTrans[firstStackIndex + j];
        } else if (htn->numSubTasks[i] == 1) {
          addListsTrans[index][0] =
          ...
          for (int l = 0; l < htn->numSubTasks[i] - 1; l++) {
            ...
            precListsTrans[index][l + pgb] =
                firstIndexTrans[firstTaskIndex + subs[l]];
            precListsTrans[index]
                          [htn->numSubTasks[i] + pgb - 1 + subs[l] - off] =
                              firstIndexTrans[firstStackIndex + subs[l]];
            addListsTrans[index][l + 1] =
                firstIndexTrans[firstTaskIndex + subs[l]] + 1 +
                subTasksInOrder[i][l + 1];
            addListsTrans[index][l + htn->numSubTasks[i]] =
                firstIndexTrans[firstStackIndex + subs[l]] + 1;

            for (int m = 0; m < pgb - 1; m++) {
              effectConditionsTrans[index][l * (pgb - 1) + m][0] =
                  firstIndexTrans[firstConstraintIndex + j * (pgb - 1) + m] + 1;
              effectsTrans[index][l * (pgb - 1) + m] =
                  firstIndexTrans[firstConstraintIndex + subs[l] * (pgb - 1) +
                                  m] +
                  1;
            }
          }
          for (int l = 0; l < htn->numOrderings[i] / 2; l++) {
            ...
            effectConditionsTrans[index][(pgb - 1) * (htn->numSubTasks[i] - 1) +
                                         l][0] =
                firstIndexTrans[firstConstraintIndex + first * (pgb - 1) +
                                second];
            effectsTrans[index][(pgb - 1) * (htn->numSubTasks[i] - 1) + l] =
                firstIndexTrans[firstConstraintIndex + first * (pgb - 1) +
                                second] +
                1;
          }
...
void HTNToSASTranslation::writeToFastDown(string sasName, bool hasCondEff,
  ...
  for (int i = 0; i < htn->gSize; i++) {
    sasfile << convertMutexVars[htn->gList[i]][0] << " "
            << convertMutexVars[htn->gList[i]][1] << endl;
  }
  for (int i = 0; i < gSizeTrans; i++) {
    sasfile << convertMutexVars[gListTrans[i]][0] << " "
            << convertMutexVars[gListTrans[i]][1] << endl;
  }
  ...
  for (int i = 0; i < numActionsTrans; i++) {
    ...
    if (primTasksIndex < tn.length()) {
      primTasks = true;
      primTaskString =
          tn.substr(primTasksIndex + 9, tn.find(": ") - 1 - primTasksIndex - 9);
    }
    int primP = 0;
    int primA = 0;
    ...
    if (primTasks) {
      int *pNum = new int[2];
      int err = calculatePrecsAndAdds(pNum, primPList, primAList,
                                      primTaskString, convertMutexVars);
      primP = pNum[0];
      primA = pNum[1];
      if (err < 0) {
        cerr << endl
             << "tasks for Method:" << endl
             << tn << endl
             << "not applicable in this order: " << primTaskString << endl;
...
void HTNToSASTranslation::planToHddl(string infile, string outfile) {
  ...
  for (int i = 0; i < linecount; i++) {
    getline(*fin, plan[i]);
    if (plan[i].size() > 9 &&
        string("primitive").compare(plan[i].substr(1, 9)) == 0) {
      primNumMax++;
    }
    if (plan[i].size() > 6 &&
        string("method").compare(plan[i].substr(1, 6)) == 0) {
      methNumMax++;
      int index = plan[i].find("firstPrim");
      if (index < plan[i].length()) {
        primNumMax += firstNumTOPrimTasks[stoi(plan[i].substr(
            plan[i].find("id") + 3, plan[i].find("]", plan[i].find("id"), 1) -
                                        plan[i].find("id") - 3))];
      }
    }
  }
  ...
  for (int i = 0; i < linecount; i++) {
    ...
    if (plan[i].size() > 9 &&
        string("primitive").compare(plan[i].substr(1, 9)) == 0) {
      primitives[primNum] = plan[i].substr(
          plan[i].find(": ") + 2, plan[i].length() - plan[i].find(": ") - 3);
      primIndex[primNum] = stoi(plan[i].substr(
          plan[i].find("id") + 3,
          plan[i].find("]", plan[i].find("id"), 1) - plan[i].find("id") - 3));
      order[i] = primNum;
      heads[i] = stoi(plan[i].substr(
          plan[i].find("head") + 5, plan[i].find("]", plan[i].find("head"), 1) -
                                        plan[i].find("head") - 5));
      ...
      primNum++;
    } else if (plan[i].size() > 6 &&
               string("method").compare(plan[i].substr(1, 6)) == 0) {
      methods[methNum] = plan[i].substr(
          plan[i].find(": ") + 2, plan[i].length() - plan[i].find(": ") - 3);
      methIndex[methNum] = stoi(plan[i].substr(
          plan[i].find("id") + 3,
          plan[i].find("]", plan[i].find("id"), 1) - plan[i].find("id") - 3));
      ...
      if (index < plan[i].length()) {
        ...
        firstPrimIndezes[methNum] =
            new int[firstNumTOPrimTasks[methIndex[methNum]]];
        string s = plan[i].substr(index + 10,
                                  plan[i].find("]", index, 1) - index - 10);
      ...
      if (index < plan[i].length()) {
        string s =
            plan[i].substr(index + 9, plan[i].find("]", index, 1) - index - 9);
      ...
      methNum++;
    } else if (plan[i].size() > 4 &&
               string("move").compare(plan[i].substr(1, 4)) == 0) {
      int from = stoi(plan[i].substr(
          plan[i].find("from") + 5, plan[i].find("]", plan[i].find("from"), 1) -
                                        plan[i].find("from") - 5));
      int to = stoi(plan[i].substr(plan[i].find("to") + 3,
                                   plan[i].find("]", plan[i].find("to"), 1) -
                                       plan[i].find("to") - 3));
      bool repl = false;
      for (int j = methNum - 1; j >= 0; j--) {
        if (repl) {
    ...
    for (int j = htn->numSubTasks[m] - 1 - fntopt; j >= 0; j--) {
      ...
      for (int k = o + 1; k < linecount - 1; k++) {
        ...
        if (order[k] < primNumMax) {
          ...
          if (subTasksInOrder[m][j] ==
              htn->decomposedTask[methIndex[order[k] - primNumMax]]) {
            if (heads[k] != subHeads[i][j]) {
              continue;
            }
            index = order[k];
...
void HTNToSASTranslation::checkFastDownwardPlan(string domain, string plan) {
  ...
  if (!fileInput->good()) {
    std::cerr << "Unable to open input file " << domain << ": "
              << strerror(errno) << std::endl;
    exit(1);
  }
  ...
  while (line.find("; cost") > 0) {
    ...
    cerr << "State" << endl;
    for (int j = 0; j < htn->numVars; j++) {
      bool correct = ((htn->precLists[index][j] == -1) ||
                      (htn->precLists[index][j] == htn->s0List[j]));
      if (!correct) {
        cerr << "cant apply method because current state is: " << htn->s0List[j]
             << " but should be: " << htn->precLists[index][j] << endl;
        return;
      }
      if (htn->precLists[index][j] != -1) {
        cerr << j << ": " << htn->s0List[j]
             << " == " << htn->precLists[index][j] << "   ";
      }
      if (htn->addLists[index][j] != -1) {
        cerr << j << ": " << htn->s0List[j] << " -> "
             << htn->addLists[index][j];
